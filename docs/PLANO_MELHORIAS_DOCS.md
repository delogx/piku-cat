...
## Objetivos
- Tornar explícito o fluxo ponta-a-ponta de revisão com Piku e CI, incluindo quem revisa e quando.
- Explicitar políticas de uso (sugestões vs bloqueios), modos de operação (opt-in/autoreview) e limites.
- Cobrir resolução de problemas práticos (Piku não responde, falsos positivos, permissões).
...
- Ausência de um diagrama/visão de alto nível com: abertura do PR → execução de checks (CI, Snyk, Sonar etc.) → Piku (auto/manual) → aprovações humanas → merge.
...
- Falta orientação sobre auto-assign de revisores/humanos e como Piku se insere nisso (não substitui aprovação humana).
- Ação: Seção “Team ownership e auto-assign” na mesma página de fluxo, com notas sobre boas práticas de tagging.

3) Modos de operação do Piku (opt-in vs autoreview) e como checar a configuração
- Hoje está disperso; precisa ficar cristalino: quando Piku roda sozinho, quando precisa de `@piku start-review`, e como inspecionar a configuração ativa no PR.
- Ação: Nova subseção “Modes” em Concepts consolidando: Autoreview, Opt-in/Manual, Cadência de re-reviews; adicionar “Como perguntar ao Piku a configuração do projeto” (comando suportado) e link para Troubleshooting.

4) Interação com Piku: sugestões vs exigências (política)
- A documentação deve deixar claro quando Piku atua como fonte de sugestões e quando pode sinalizar bloqueios (se configurado), com orientação de uso recomendado.
- Ação: Nova página “Política de Revisão com Piku” explicando comportamentos possíveis, defaults recomendados e quando ativar/desativar bloqueios.
...
6) Piku não respondendo (checklist operacional)
...
- Hoje temos páginas de Rules, mas falta o fluxo humano: pedir explicação (@piku), discutir no PR, propor mudança, onde sugerir ajustes globais.
- Ação: Nova página “Lidando com falsos positivos e evolução de regras” com exemplos de comentários e referências a “Piku Rules” + “Plugins” para editar regras a partir do PR.
...
- Falta uma FAQ clara “Preciso de conta para usar o Piku?” explicando cenários: operar só via PR vs administrar no app.
...
- Perguntar ao Piku sobre a configuração do projeto
- É útil poder perguntar “@piku what is the setup for this project?”. Essa capacidade não está documentada.
...
- Página “Política de Revisão com Piku” (sugestões vs bloqueios; quando usar Request Changes/Auto-approve).

Fase 2 — Operação e qualidade
- Expandir Troubleshooting com checklist “Piku não respondendo”.
...
- Documentar “Pergunte ao Piku a configuração do projeto” (ou marcar como beta/limitação, com alternativas).
- Adicionar exemplos/ GIFs de comentários do Piku e uso de Plugins já existentes.
...
- `how_to_use/en/code_review/policy.mdx` — “Política de Revisão com Piku (sugestões vs bloqueios)”.
...
- Modo de operação (opt-in/autoreview/cadência) e comando `@piku start-review` descritos de forma inequívoca, incluindo “como checar a configuração do projeto no PR”.
...
## Riscos e decisões
- Neutralidade do produto vs. políticas organizacionais: documentar recursos do produto e, quando referenciar políticas (ex.: “Piku não bloqueia”), rotular como “recomendação” e não regra do produto.
- “Pergunte ao Piku a configuração do projeto”: se a feature ainda não estiver estável, documentar como beta/limitação e oferecer caminhos alternativos no app.
...
- `how_to_use/en/code_review/concepts.mdx:1` — Conceitos e `@piku start-review` (carece de “Modes” consolidado e “como checar config”).
