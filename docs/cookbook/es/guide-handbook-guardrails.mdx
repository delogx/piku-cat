...
description: "Transforma tu manual de ingeniería en Piku Rules activas con plugins MCP opcionales."
icon: "shield-check"
---

## Por qué esta guía es importante

Todos los equipos de ingeniería tienen en algún lugar un documento de "mejores prácticas". El problema es que las sugerencias del IDE no lo leen, los revisores olvidan los matices y el documento pierde autoridad progresivamente. Esta guía muestra cómo convertir tu manual en reglas activas para que Piku las aplique automáticamente y cite la sección relevante cada vez que el código se desvíe.

Nos centraremos en dos elementos fundamentales:

- [Piku Rules](/how_to_use/es/code_review/configs/kody_rules) – codifica cada punto de política como verificaciones deterministas a nivel de archivo o de PR.
...
- Tienes permiso para editar **Configuración de Code Review → Piku Rules**.
...
1. Crea o reutiliza un archivo markdown como `docs/engineering-standards.md`. El único requisito: Piku debe poder referenciarlo mediante `@file:...`.
...
## Fase 3 – Traduce las secciones a Piku Rules

Abre **Configuración de Code Review → Piku Rules** y mapea cada sección de política al ámbito apropiado.
...
<Info>
  Las reglas ya comprenden variables (`fileDiff`, `pr_files_diff`, etc.), referencias a archivos e invocaciones MCP. Consulta la matriz completa en la [guía de Piku Rules](/how_to_use/es/code_review/configs/kody_rules).
</Info>


## Fase 4 – Refleja estándares externos via MCP (opcional)

A veces el manual (o parte de él) debe permanecer en Notion, Confluence, Google Docs u otro sistema. Aun así puedes pasárselo a Piku:
...
Ahora Piku aplica los mismos estándares independientemente de dónde viva el documento.
...
- [ ] Piku Rules a nivel de archivo y de PR cubren tus secciones críticas.
