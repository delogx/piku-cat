...
description: "Converta seu handbook de engenharia em Piku Rules vivas com plugins MCP opcionais."
icon: "shield-check"
---

## Por que este guia é importante

Todo time de engenharia tem um documento de "boas práticas" em algum lugar. O problema é que as sugestões do IDE não o leem, os revisores esquecem os detalhes, e o documento vai perdendo autoridade aos poucos. Este guia mostra como transformar seu handbook em guardrails ativos para que o Piku aplique essas regras automaticamente e cite a seção relevante sempre que o código se desviar.

Vamos focar em dois blocos fundamentais:

- [Piku Rules](/how_to_use/pt-br/code_review/configs/kody_rules) – codifique cada item de política como verificações determinísticas em nível de arquivo ou de PR.
...
- Você tem permissão para editar **Configurações de Code Review → Piku Rules**.
...
1. Crie ou reutilize um arquivo markdown como `docs/engineering-standards.md`. O único requisito: o Piku deve conseguir referenciá-lo via `@file:...`.
...
## Fase 3 – Traduza seções em Piku Rules

Abra **Configurações de Code Review → Piku Rules** e mapeie cada seção de política para o escopo adequado.
...
<Info>
  As regras já entendem variáveis (`fileDiff`, `pr_files_diff`, etc.), referências de arquivo e invocações MCP. Veja a matriz completa no [guia de Piku Rules](/how_to_use/pt-br/code_review/configs/kody_rules).
</Info>


## Fase 4 – Espelhe padrões externos via MCP (opcional)

Às vezes, o handbook (ou parte dele) precisa permanecer no Notion, Confluence, Google Docs ou outro sistema. Você ainda pode alimentá-lo no Piku:
...
Agora o Piku aplica os mesmos padrões independentemente de onde o documento esteja.
...
- [ ] Piku Rules em nível de arquivo e de PR cobrem suas seções críticas.
