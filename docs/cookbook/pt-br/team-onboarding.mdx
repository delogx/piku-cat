...
description: "Guia passo a passo para configurar o piku-cat para um novo time — desde a conexão de repositórios até o ensino das primeiras convenções."
icon: "users-gear"
---

## Visão geral

Colocar um novo time em funcionamento com o piku-cat leva cerca de 30 minutos. Este guia percorre a configuração completa para que seu time extraia valor desde o primeiro PR.
...
1. Vá para **Configurações de Code Review** → **Piku Rules**
2. O piku-cat detecta arquivos `.cursorrules`, `.github/copilot-instructions.md` e `.claude`
...
Conecte Jira, Linear, Notion ou ClickUp em **Configurações** → **Plugins** para habilitar a validação de lógica de negócio. Isso permite que o piku-cat verifique se os PRs implementam o que as tarefas descrevem.

## Passo 7 — Execute um PR piloto

Abra um PR de teste e verifique:

- [ ] Os comentários do piku-cat aparecem no PR
...
- Quer regras geradas automaticamente? → Clique em **Gerar Piku Rules** para obter sugestões do histórico de revisões

## Dicas

- Não sobrecarregue com regras no primeiro dia — comece pequeno e expanda
- Deixe o time ensinar o Piku naturalmente por meio de conversas antes de formalizar como regras
- Use `@kody start-review` nos primeiros PRs para que o time veja o piku-cat em ação
- Habilite a aprovação de memórias geradas por LLM se quiser revisar o que o Piku aprende
