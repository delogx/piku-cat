---
title: "新团队入门"
description: "分步指南：为新团队设置 piku-cat — 从连接代码库到教授首批约定。"
icon: "users-gear"
---

## 概述

为新团队启动 piku-cat 大约需要 30 分钟。本指南介绍完整设置，使您的团队从第一个 PR 开始就能获得价值。
...
1. 前往**代码审查设置** → **Piku 规则**
2. piku-cat 检测 `.cursorrules`、`.github/copilot-instructions.md` 和 `.claude` 文件
3. 导入对代码审查有意义的规则
...
在**设置** → **插件**中连接 Jira、Linear、Notion 或 ClickUp 以启用业务逻辑验证。这让 piku-cat 检查 PR 是否实现了任务描述的内容。
...
- 规则太嘈杂？→ 缩小文件路径或添加排除
- 遗漏重要模式？→ 添加新规则或记忆
- 想要自动生成规则？→ 点击**生成 Piku 规则**从审查历史中获取建议

## 提示

- 第一天不要过度添加规则 — 从小处开始逐步增长
- 让团队在正式化为规则之前通过对话自然地教 Piku
- 前几个 PR 使用 `@kody start-review` 让团队看到 piku-cat 的实际效果
- 如果想审查 Piku 学到的内容，启用 LLM 生成记忆审批
