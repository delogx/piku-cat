...
description: "Manually register the Azure Repos webhook when Piku couldn't create the service hook for you, including the signed token the URL requires."
icon: webhook
---

Piku usually creates the webhook automatically when you connect an Azure DevOps project. This page covers the **manual setup** path you take when automatic registration didn't happen — typically on self-hosted installs or in Azure projects where Piku can't create service hooks on your behalf.
...
<Note>
  Piku generates this token automatically when it creates the webhook for you via the UI-driven integration flow. You only need to generate it by hand when you're registering the webhook manually in Azure DevOps.
</Note>

## Generating the webhook token

Pick the option that matches your environment. Both use the env vars that your piku-cat stack already has configured — you do **not** need to clone the repository.

### Option A — inside the running piku-cat container (recommended)
...
If your piku-cat stack is not running or you want to generate the token on a different machine, export the two env vars and run:
...
<Warning>
  Both env var values must match **exactly** the ones your piku-cat API container is using. A key mismatch produces a valid-looking token that piku-cat will reject with `403 Unauthorized`.
</Warning>
...
5. Configure the webhook:
   ...
   - **URL:** your piku-cat Azure Repos webhook URL with the token appended:
...
Create one subscription per event type. piku-cat currently reacts to:
...
<AccordionGroup>
  <Accordion title="Azure sends the webhook but piku-cat returns 403">
