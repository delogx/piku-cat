...
description: "Manually create the Forgejo webhook when automatic registration didn't happen, and select the events piku-cat consumes."
icon: webhook
---
...
piku-cat usually creates the webhook automatically. If that didn't happen, follow these steps to set it up manually:
...
<Note>
  piku-cat consumes pull request, push, pull request review comment, and issue comment events.
  Push events matter on Forgejo specifically: they are the only event carrying both the previous
  and current branch heads, which is how piku-cat detects force-pushes and maps a branch back to its
