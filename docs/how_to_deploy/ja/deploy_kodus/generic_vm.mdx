---
title: "インストール"
description: "汎用仮想マシンにpiku-catをデプロイする。"
icon: "plug"
---
...
これらのサービスはオプションですが、piku-catのコードレビューBIG品質を大幅に向上させます。それぞれのサービスでアカウントを作成してAPIキーを生成する必要があります。

<AccordionGroup>
  <Accordion title="E2B — コード実行サンドボックス">
    E2Bはpiku-catがレビュー中にコードを実行・検証するために使用するセキュアなサンドボックスを提供します。
  ...
  <Accordion title="MorphLLM — コード編集モデル">
    MorphLLMは高速かつ正確なコード編集のための特化モデルです。piku-catはこれをRevvisionBの提案をより正確に適用するために使用します。
