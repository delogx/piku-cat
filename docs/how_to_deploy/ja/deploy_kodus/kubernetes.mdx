...
description: "公式 Helm チャートで piku-cat を Kubernetes または OpenShift にデプロイします。"
icon: "dharmachakra"
---

単一の VM よりもクラスターを使いたい場合は、piku-cat が
...
`imageTag` はスタック全体の piku-cat バージョンを固定します（Docker Compose の `IMAGE_TAG`
