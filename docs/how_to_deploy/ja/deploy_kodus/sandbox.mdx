...
description: "piku-cat がリポジトリをどうパースするか（kodus-graph）、どこで実行されるか（サンドボックスモード）、そして各モードをいつ選ぶか。"
icon: "box"
---

PR を適切なコンテキストでレビューするため、piku-cat はリポジトリ全体を
...
piku-cat は [`@kodus/kodus-graph`](https://www.npmjs.com/package/@kodus/kodus-graph)
...
1. **リポジトリを選択したとき**（セットアップ時、または後で UI で
   リポジトリを追加するとき） — piku-cat は `AstGraphBuild` ジョブを
...
2. **PR がデフォルトブランチにマージされたとき** — webhook ハンドラー
   ...
   でキャッシュが新鮮に保たれます。
3. **すべての PR レビュー時** — piku-cat はキャッシュされたグラフを
...
実行されるため、完了を待たずに piku-cat を使い続けられます。
