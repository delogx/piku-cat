---
title: GitHub Appの作成
description: "Piku がリポジトリを読み、プルリクエストにコメントし、Webhook を受け取るための GitHub App を作成する手順。付与すべき権限も明記します。"
icon: github
---

PikuをGitHub Appで実行する場合は、アプリを作成する必要があります。このアプリにより、Pikuはリポジトリとやり取りし、プルリクエストを管理し、Webhookを受信できるようになります。
...
- オプション**Redirect on update**を有効にする
  これにより、アプリのインストール後にユーザーがpiku-catにリダイレクトされてセットアップを完了できるようになります。

## ステップ3：Webhookの設定

Webhookはpiku-catにプルリクエストのイベントを通知します。
...
GitHubが生成した認証情報を取得し、piku-catの環境変数に追加する必要があります。
...
GitHubで**Save Changes**をクリックし、piku-catアプリケーションを再起動して変更を適用してください。
