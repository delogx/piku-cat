---
title: "Proxy Reverso"
description: "Configure um proxy reverso para o piku-cat."
icon: "server" 
...
Para tornar sua instância do piku-cat acessível a partir de sistemas externos (necessário para webhooks de Code Review e acesso público), você precisará configurar um proxy reverso com Nginx.

Aqui está um exemplo de configuração Nginx para dois subdomínios:

```nginx
# Bloco de servidor para a Aplicação Web do piku-cat
server {
    listen 80;
    server_name kodus-web.yourdomain.com; # Substitua pelo domínio real da sua aplicação web

    location / {
        proxy_pass http://localhost:3000; # Aponta para o contêiner da Aplicação Web do piku-cat
...
# Bloco de servidor para a API do piku-cat
server {
    listen 80;
    ...
    location ~ ^/(github|gitlab|bitbucket|azure-repos)/webhook {
        proxy_pass http://localhost:3332; # Aponta para o serviço de Webhooks do piku-cat
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
    ...
    location / {
        proxy_pass http://localhost:3001; # Aponta para o serviço de API do piku-cat
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
