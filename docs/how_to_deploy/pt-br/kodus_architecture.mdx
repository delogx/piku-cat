---
title: "Arquitetura do piku-cat"
description: "Uma análise aprofundada da infraestrutura técnica do piku-cat."
icon: "puzzle-piece"
---

## Visão Geral

Este documento descreve a arquitetura que sustenta a infraestrutura do piku-cat. Nosso sistema é construído sobre uma arquitetura distribuída que utiliza contêinerização e segmentação de rede para garantir máxima escalabilidade, segurança e manutenibilidade.
...
- Analytics Worker: Worker opcional de ingestão (`WORKER_ROLE=analytics`) que alimenta os dashboards do Cockpit. Apenas Enterprise — veja [Analytics Worker](/how_to_deploy/pt-br/deploy_kodus/analytics_worker)

### 3. MCP Manager

O MCP Manager cataloga provedores e integrações, e os disponibiliza para o piku-cat para que
as equipes possam instalar MCPs pela tela de Plugins.

### 4. Armazenamentos de Dados

O piku-cat utiliza dois bancos de dados:
...
### 6. Serviços Auxiliares (piku-cat Cloud)

O piku-cat Cloud inclui serviços auxiliares de código fechado (faturamento e
...
## Próximos Passos
<CardGroup cols={2}>
  <Card title="Executar o piku-cat Localmente" icon="laptop" href="/how_to_deploy/pt-br/local_quickstart/orchestrator">
    Ideal para desenvolvimento local e para se familiarizar com o stack completo do piku-cat.
  </Card>
  <Card title="Implantar o piku-cat em Produção" icon="rocket" href="/how_to_deploy/pt-br/deploy_kodus/generic_vm">
    Perfeito para implantação em produção e para aproveitar todas as capacidades do piku-cat.
  </Card>
</CardGroup>
