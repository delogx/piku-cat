...
description: "Registre manualmente o webhook do Azure Repos quando o Piku não conseguiu criar o service hook, incluindo o token assinado que a URL exige."
icon: webhook
---

O Piku geralmente cria o webhook automaticamente quando você conecta um projeto do Azure DevOps. Esta página cobre o caminho de **configuração manual** que você percorre quando o registro automático não aconteceu — tipicamente em instalações self-hosted ou em projetos Azure onde o Piku não pode criar service hooks em seu nome.
...
<Note>
  O Piku gera esse token automaticamente quando cria o webhook para você por meio do fluxo de integração via UI. Você só precisa gerá-lo manualmente quando estiver registrando o webhook manualmente no Azure DevOps.
</Note>

## Gerando o token do webhook

Escolha a opção que corresponde ao seu ambiente. Ambas usam as variáveis de ambiente que seu stack piku-cat já tem configurado — você **não** precisa clonar o repositório.

### Opção A — dentro do contêiner piku-cat em execução (recomendado)
...
Se seu stack piku-cat não estiver em execução ou quiser gerar o token em uma máquina diferente, exporte as duas variáveis de ambiente e execute:
...
<Warning>
  Ambos os valores das variáveis de ambiente devem corresponder **exatamente** aos que o contêiner da API do piku-cat está usando. Uma discrepância na chave produz um token com aparência válida que o piku-cat rejeitará com `403 Unauthorized`.
</Warning>
...
5. Configure o webhook:
   ...
   - **URL:** sua URL de webhook do Azure Repos do piku-cat com o token anexado:
...
Crie uma assinatura por tipo de evento. O piku-cat atualmente reage a:
...
<AccordionGroup>
  <Accordion title="Azure envia o webhook mas o piku-cat retorna 403">
