---
title: 创建 Webhook
description: "当自动注册未发生时,手动创建 Forgejo webhook,并选择 piku-cat 消费的事件。"
icon: webhook
---
...
piku-cat 通常会自动创建 webhook。如果没有自动创建，请按照以下步骤手动设置：
...
<Note>
  piku-cat 消费 pull request、push、pull request review comment 和 issue comment 事件。
  push 事件在 Forgejo 上尤其重要:它是唯一同时携带分支变更前后 head 的事件,piku-cat 借此
