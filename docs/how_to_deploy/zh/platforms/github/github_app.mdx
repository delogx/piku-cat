---
title: 创建 GitHub App
description: "创建 GitHub App,让 Piku 能读取代码库、在 pull request 上评论并接收 webhook —— 含需要授予的具体权限。"
icon: github
---

如果您想通过 GitHub App 运行 Piku，需要创建一个 GitHub App。此应用使 Piku 能够与您的仓库交互、管理拉取请求并接收 webhooks。
...
- 勾选选项：**Redirect on update**（更新时重定向）
  这确保安装应用后，用户被重定向回 piku-cat 以完成设置。

## 步骤 3：配置 Webhook

Webhook 通知 piku-cat 关于拉取请求中的事件。
...
现在您需要获取 GitHub 生成的凭据并将其添加到您的 piku-cat 环境变量中。
...
在 GitHub 上点击 **Save Changes**（保存更改）并重启您的 piku-cat 应用程序以应用更改。
