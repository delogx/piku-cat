...
description: "创建为 piku-cat 用户启用「Sign in with GitLab」的 GitLab OAuth App —— 与用于代码审查的 webhook 集成相互独立。"
icon: gitlab
---

如果您希望用户通过 GitLab 登录 piku-cat，请使用此方式。这与用于代码审查自动化的 GitLab Webhook 集成是分开的。
...
创建完成后，从 GitLab 复制凭据并写入 piku-cat 环境变量：
