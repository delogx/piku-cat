...
description: "Configure your own API keys for maximum flexibility and cost control. Available on every piku-cat plan."
---

BYOK (Bring Your Own Key) is the **default way piku-cat uses LLMs across every plan** — Community, Teams, and Enterprise. You connect your own provider account, pick a model, pay only for what you use, and monitor costs directly on your provider's dashboard. piku-cat never marks up tokens and never sees your key in plain text.
...
>
  BYOK is free on Community, included on Teams ($10/active dev/month on top of your token spend), and one of two options on Enterprise (the other being a piku-cat-managed API key).
</Card>
...
<Steps>
  ...
  <Step title="Add a Fallback (recommended)">
    Once the Main model is configured, a **Fallback model** section appears. If your main provider hits rate limits or goes down, piku-cat falls back automatically.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="GLM 5.1 (Z.ai)">
    ...
    <Warning>
      GLM Coding Plan keys **only** work on `/api/coding/paas/v4`. The Lite and Pro tiers are often capped at **1 concurrent request** — piku-cat pre-fills `maxConcurrentRequests=1` when you pick this plan. Bump it in Advanced settings if you're on the Max tier (up to 30).
    </Warning>
  </Tab>
  ...
  <Tab title="Kimi K2.6 (Moonshot AI)">
    ...
    <Info>
      Kimi Code Plan is documented at a cap of 30 concurrent requests. piku-cat pre-fills `maxConcurrentRequests=30` when you pick that plan.
    </Info>
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Pick or type the model ID">
    If piku-cat can list models from the provider, you get a dropdown. Otherwise (e.g. self-hosted or when platform keys aren't configured), type the exact model ID manually.
  </Step>
...
<Tabs>
  <Tab title="OpenAI">
    ...
    **Get an API key:**
    ...
    2. Create a new key for piku-cat
  ...
  <Tab title="Google Vertex AI">
    ...
    **How to configure:**
    ...
    2. Paste the **contents of your service account JSON file** into the Service Account JSON
       field (base64-encoded also works). piku-cat extracts `project_id` from the JSON
       automatically.
  ...
  <Tab title="Anthropic-compatible">
    ...
    **How to configure:**
    ...
    <Note>
      ...
      `/v1` (e.g. `https://api.kimi.com/coding` or `https://api.kimi.com/coding/v1`). piku-cat
...
When you pick Low / Medium / High, piku-cat translates the level to each provider's native format automatically:
...
<Note>
  Kimi and GLM currently expose reasoning as a single on/off flag. Picking Low, Medium, or High all emit the same payload (thinking enabled). When their APIs add level granularity, piku-cat will start forwarding it.
</Note>

### Custom JSON override

Picking **Custom** in the Thinking toggle reveals a JSON textarea. Paste the provider options directly — **piku-cat auto-wraps them under the active provider's namespace**. You don't need to know the Vercel AI SDK routing rules.
...
- The provider ships a new field piku-cat hasn't wrapped yet
...
If your JSON already contains a known namespace key at the top level (`anthropic`, `google`, `openai`, `openrouter`, `openaiCompatible`), piku-cat leaves it untouched. Useful if you want to mix multiple provider namespaces or be explicit:
...
Under the hood, these are the namespace mappings piku-cat uses:
...
- **Valid JSON only.** Missing commas or trailing commas break the parse and piku-cat ignores the override.
...
- **Unknown provider = no wrap.** If your BYOK provider isn't in the namespace table above, piku-cat passes the JSON through as-is. Rare — only applies if you configure a provider piku-cat doesn't recognize.
...
Under the hood, piku-cat emits this into the Vercel AI SDK call:
...
The `maxConcurrentRequests` field (under **Advanced settings**) caps how many inflight requests piku-cat sends to your provider in parallel. Most of the time, the default is fine — but subscription plans with strict concurrency caps need it set explicitly.

### Defaults piku-cat pre-fills
...
| **Anthropic / OpenAI / Google / OpenRouter** | _(empty)_ | Providers enforce their own TPM/RPM; piku-cat doesn't cap. |
...
<Note>
  **Fallback interaction.** When Main hits a 429 and piku-cat fails over to the Fallback model, the Fallback's own `maxConcurrentRequests` applies. Setting a generous Fallback on a different provider is a good way to absorb bursts when your Main is on a tight subscription.
</Note>
...
<CardGroup cols={2}>
  <Card title="Dedicated Keys" icon="shield-check">
    Create separate API keys for piku-cat. Makes usage auditing and key rotation easier.
  </Card>
  ...
  <Card title="Secure Storage" icon="lock">
    Never commit keys to repositories. piku-cat stores them encrypted at rest and in transit.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Self-hosted env vars not showing">
    - If piku-cat is configured via `.env` (self-hosted Fixed Mode), the BYOK screen shows a blue info banner with the active provider/model — the key is never displayed for security.
...
<AccordionGroup>
  ...
  <Accordion title="How does the primary/fallback system work?">
    Main handles every review by default. If it fails (rate limit, 5xx, timeout), piku-cat retries once on Fallback. You pay only for the provider that actually processed the review.
  </Accordion>
