...
3. Deploys piku-cat skills to every detected agent (Claude Code, Cursor, Codex, Windsurf).
...
With the skill installer, Claude Code's piku-cat skill knows when to invoke each of these — no prompt engineering required on your side.
