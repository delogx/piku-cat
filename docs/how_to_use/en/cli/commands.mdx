...
| `--github-pat <token>` | —     | GitHub token (`read:repo`) used to clone private repos. Trial users only. Also settable via `KODUS_GITHUB_PAT`. Held in memory only, never persisted. |
...
Manage authentication with the piku-cat API.
...
Removes the pre-push hook installed by piku-cat.
...
Inspect and update repository settings stored in piku-cat. Team-key auth is required. Use `.` to target the current repository.
...
```bash
kodus config repo add                                   # Add current repo to piku-cat
...
kodus config repo open                                  # Open the piku-cat dashboard
...
| Subcommand                                         | Description                                                                 |
| -------------------------------------------------- | --------------------------------------------------------------------------- |
| `add [repository]`                                 | Add a repository to piku-cat (shortcut: `kodus config -r [repository]`)       |
| `list`                                             | List repositories configured in piku-cat                                       |
...
| `open [repository]`                                | Open the piku-cat dashboard for advanced settings                              |
...
Create, update, and view Piku Rules — structured rules that piku-cat applies during review.
...
| Subcommand | Description            |
| ---------- | ---------------------- |
| `create`   | Create a new Piku Rule |
| `update`   | Update an existing rule |
| `view`     | View Piku Rules        |
...
Show a consolidated piku-cat status: auth mode, organization, repository, hook, bundled skills, and CLI version.
...
<Note>
  Useful for eyeballing a changeset before running `kodus review` on it. The same viewer backs
  the interactive review experience; pass `--no-hunk` to `kodus review` if you prefer the
  legacy list.
</Note>
...
Opens the piku-cat pricing/upgrade page in your browser.
...
| `documentation`          | Documentation gaps (accepted by `--category` on `kodus pr suggestions`) |
