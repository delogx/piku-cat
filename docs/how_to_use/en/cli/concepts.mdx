...
A **Piku Rule** is a structured rule the piku-cat reviewer applies during every review, scoped to a repository or global to your team.
...
Rules layer on top of the general piku-cat review — the AI uses them as additional criteria, not replacements.
...
Every repository in piku-cat has settings that govern how reviews behave: ignored files, base branch patterns, title filters, feature toggles. These live in the piku-cat backend and are mirrored anywhere the repo is reviewed (web PRs, CLI reviews, agent loops).
...
Normally, each repository's settings live in piku-cat. **Centralized Config** lets a team store those settings in a single source-of-truth git repository — every change becomes a pull request against that repo, giving you review and version history on your review configuration itself.
...
Use Centralized Config when your team wants auditable, versioned control over review settings. Keep the default (per-repo settings in piku-cat) when that ceremony is overkill.
