...
Kodus CLI reviews your code changes directly from the terminal — catching bugs, security issues, and style violations before they reach a pull request. It's the same piku-cat engine that runs on your PRs, but pointed at your working tree, staged diff, branch, or commit.
...
<CardGroup cols={2}>
  ...
  <Card title="PR Suggestions" icon="code-pull-request">
    Fetch Piku's suggestions on an open pull request without leaving your terminal.
  </Card>
  ...
  <Card title="Piku Rules" icon="scale-balanced">
...
The CLI and the piku-cat web dashboard share the same review engine, rules, and repository settings. Use the CLI when you want reviews **before the PR exists** — on a local branch, a staged diff, or inside an agent loop. Use the web dashboard for PR reviews, metrics, and team-level configuration.
...
<CardGroup cols={2}>
  ...
  <Card title="Self-Hosted" icon="server" href="self_hosted">
    Connect the CLI to your own piku-cat instance.
  </Card>
