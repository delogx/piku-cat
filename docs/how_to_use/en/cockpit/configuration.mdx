...
| Tab | What it holds |
| --- | --- |
| **piku-cat Review** | How effective piku-cat's reviews are — implementation rate, severity calibration, negative feedback, Piku Rules health. |
...
- New metrics ship **visible by default**. If piku-cat adds a chart, it appears for everyone until
...
- Hiding a metric only affects display. piku-cat keeps ingesting the underlying data, so turning it
