...
description: "Measure how effective piku-cat's reviews are — and act on it"
---

## What is the piku-cat Review tab

The **piku-cat Review** tab answers one question: *is your team acting on what piku-cat says?*

Unlike the Productivity tab (which measures general delivery metrics like deploy frequency and PR cycle time), piku-cat Review is about piku-cat itself — how many suggestions get implemented, which categories and rules land or get ignored, and where the team pushes back.
...
| **Suggestions sent** | How many suggestions piku-cat delivered (as PR comments) in the period |
...
- Only **delivered** suggestions count (the ones actually posted as PR comments). Drafts piku-cat filtered out before commenting are never counted.
...
<Warning>
  **The "All / piku-cat only" toggle on the severity chart matters.** A Piku Rule carries the severity *you* set on the rule, not a risk assessment by piku-cat. Mixing the two distorts the calibration read — for example, a pile of medium Piku Rules with high adoption can make piku-cat look like it under-rates medium. Switch to **piku-cat only** to see piku-cat's own severity calibration. Bars built on very few suggestions are faded and marked with `*` — a 0% or 100% from a handful of suggestions is not a real signal.
</Warning>

## Negative feedback

Feedback comes from 👍 / 👎 reactions on piku-cat's suggestion comments.
...
A per-repository table: PRs reviewed, suggestions sent, implementation rate, 👍/👎, and the **weakest category** (the category with the lowest implementation rate in that repo, given a minimum sample). It shows where piku-cat is landing versus being ignored.

Clicking a row focuses the whole cockpit on that repository (same as picking it in the repository filter).

## Piku Rules — health

How each Piku Rule is performing in the period: triggers, implementation rate, 👍/👎, and a status. Only active rules appear — deleted or inactive ones are excluded since you can't act on them.
...
Filters: repository, category, severity, implementation status, Piku Rule, and free-text search. Each row expands to show the existing vs. suggested code and a link to the PR comment.
