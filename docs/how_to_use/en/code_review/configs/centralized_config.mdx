...
description: "Manage all code review configuration and Piku Rules as code in a single repository. Changes sync automatically after each merge; rollout is controlled from the UI or the CLI."
---

Centralized Config lets you keep every code review setting and every Piku Rule in **one repository** instead of editing them per-repo in the web UI. Each change flows through a pull request on that repository, giving you version history, review, and rollback on your review configuration.
...
- Consistent Piku Rules (review rules and memories) across the whole organization.
- CLI-driven or CI-driven configuration changes.

## How It Works

1. Choose one repository to hold your centralized config. It has to be one of the repositories selected in piku-cat.
2. Enable Centralized Config in the UI and point it at that repository.
3. Pick an **initial sync mode**:
   - `pr` — piku-cat opens the initialization PR with your current effective settings and rules.
   - `manual` — piku-cat generates the files; your team downloads and opens the PR.
4. After the first merge, every future merge on the centralized repository automatically propagates changes to piku-cat.

The centralized repository mirrors piku-cat's scope hierarchy (Global → Repository → Directory) through its folder layout.

## Repository Layout

The centralized repository uses folder paths to map files to scopes. The layout covers both review settings (`kodus-config.yml`) and Piku Rules (`.kody-rules/`).
...
│
├── backend-api/                          ← matches a piku-cat repo
...
- A top-level folder whose name matches a piku-cat repository (e.g. `backend-api/`) scopes files to that repository.
...
Each rule is a single YAML file under `.kody-rules/review/` (standard rules) or `.kody-rules/memories/` (memories). The filename is arbitrary — piku-cat identifies rules by the `title` field and by the canonical file path stored with the rule.
...
See [Piku Rules Overview](/how_to_use/en/code_review/configs/kody_rules) for the full field reference and [Rule Inheritance](/how_to_use/en/code_review/configs/rules_inheritance) for how `inheritance.inheritable`, `exclude`, and `include` behave.
...
- **Piku Rules and PR Messages** remain editable in the web UI. Saving an edit does not mutate the rule directly — piku-cat opens a pull request on the centralized repository with the proposed change. Once the PR is merged, the change becomes active.
...
- At review time, piku-cat loads **every active rule** from the database regardless of origin. A rule that exists only in the UI is applied exactly like a rule that exists only in the repo.
...
| Status | Meaning |
|---|---|
| `SYNCED` | The rule in piku-cat matches the file in the centralized repository. |
...
<Tabs>
  <Tab title="PR (default)">
    piku-cat opens the initialization pull request automatically with your
    current effective settings and rules.
  </Tab>
  <Tab title="Manual">
    piku-cat generates the files but does not open a PR. Use the download bundle
...
Rules and settings that were synced in remain in piku-cat — they are not deleted on disable.
...
- At least one selected repository in piku-cat.
...
- [Piku Rules Overview](/how_to_use/en/code_review/configs/kody_rules)
