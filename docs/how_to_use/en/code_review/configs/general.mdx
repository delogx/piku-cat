...
<Note>
  piku-cat has three config levels: Global, Repository, and Directory. Learn how they work in <a href="/how_to_use/en/code_review/configs/inheritance_overrides">Config Inheritance & Overrides</a>.
</Note>
...
<Warning>
  **The file is ignored until you opt in.** By default `kodusConfigFileOverridesWebPreferences`
  is `false`, which means Piku never reads `kodus-config.yml` — committing the file alone does
  nothing.
...
Piku can review PRs automatically when opened, or only when you comment `@kody start-review`.
...
How Piku handles reviews on subsequent commits:
...
#### Auto-approve PRs
Piku approves PRs when no issues are found.

**Platforms:** GitHub, GitLab, Bitbucket

#### Request Changes
Piku requests changes when finding critical issues.
...
<Info>
  **Empty (the default) means every base branch is reviewed** — there is no built-in
  ...
  list is non-empty, Piku reviews only PRs targeting those branches (your repository's default
...
Piku reviews across four categories. Enable or disable each one under **Analysis Types**, or via
...
<Note>
  Piku Rules are **not** one of these categories — they always run, independently of
  `reviewOptions`. See [Piku Rules](/how_to_use/en/code_review/configs/kody_rules).
</Note>
