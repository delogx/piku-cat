---
title: "Overview"
description: "Piku Rules are customizable guidelines your team sets up to automatically enforce code quality, consistency, security, and maintainability."
---
...
## Two Types in the Piku Rules UI

Piku Rules are organized into two categories, managed through a tabbed interface in your settings:
...
<Tabs>
  ...
  <Tab title="Memories">
    Persistent contextual instructions that are injected across all prompts and conversations. Memories allow Piku to learn and retain how your codebase works so future reviews and suggestions are more relevant.

    - Injected as **high-priority context** in code reviews, cross-file analysis, and conversations
    - Created via **conversation with Piku** or **manually in the UI**
...
- Piku identifies file references automatically when you save a rule
...
- File contents are injected into the rule context, allowing Piku to compare, validate, or enforce patterns
...
- Navigate to Discovery Rules in your piku-cat dashboard.
...
Memories are persistent contextual instructions that Piku learns from your team's conversations and coding practices. Unlike Review Rules that run during code review, Memories are injected across all prompts and conversations to provide continuous, high-priority context and improve future suggestions.
...
- **Better future suggestions**: Memories help Piku avoid repeating the same mistaken assumptions about your repo
- **Intelligent deduplication**: When a new memory is created, Piku uses an LLM-based resolution mechanism to determine whether to create it, skip it (if duplicate), or update an existing memory
...
The most natural way to create memories is through conversation with Piku in your PR comments. Piku detects both explicit and implicit intents to save conventions:

**Explicit** — directly asking Piku to remember:
...
**Implicit** — stating a preference or convention that Piku captures:
...
<Info>
  Piku will NOT create memories for transient instructions (e.g., "fix this now"), debugging chatter, questions, vague statements, or requests explicitly scoped to a single task or PR.
</Info>

When a memory is created or updated, Piku responds with a confirmation and provides a **direct link** to view the memory in the UI.

#### Via the UI

You can also create memories manually:

1. Go to **Code Review Settings** → **Repository** → **Piku Rules**
...
1. Go to **Code Review Settings** → **Repository** → **Piku Rules** → **Memories** tab
2. Enable **Piku knowledge approval**
