...
description: "Automatically detect and import your existing rule files from popular AI coding tools to enhance Piku's code review capabilities with your team's established coding standards."
---

piku-cat can automatically sync and import your existing IDE rule files to enhance Piku's code review with your team's established coding standards and preferences.
...
Rules File Detection automatically detects and imports configuration files from popular AI coding tools and assistants in your repository. This ensures Piku understands and respects your existing coding standards without requiring manual setup.

## Supported Rule Files

piku-cat automatically detects and imports the following rule file patterns:
...
- `.kody/rules/**` and `rules/**/*.md` - Piku-specific rules (see [Repository Rules](/how_to_use/en/code_review/configs/repository_rules) — these are imported verbatim, without LLM rewriting)
...
3. **Repository Scan**: When you confirm, piku-cat scans your entire repository for supported rule files
4. **Rule Generation**: An LLM processes the found files and automatically creates corresponding Piku Rules. Exception: `.kody/rules/**` and `rules/**/*.md` template files are imported **verbatim** — no LLM conversion — so your instructions, examples and identifiers are preserved exactly as authored. If one of those files doesn't follow the template (missing/invalid frontmatter, or no `title`), it falls back to the LLM path.

Each imported file produces **one Piku Rule**. If a file contains several distinct guidelines, the LLM merges them into a single comprehensive rule — split guidance across multiple files when you want independently manageable rules.

### Ongoing Synchronization
- **Automatic Updates**: Sync happens automatically when a Pull Request is closed
- **File Changes Detected**: 
  - **New rule files** → Creates new Piku Rules
  - **Modified rule files** → Updates existing Piku Rules  
  - **Deleted rule files** → Removes corresponding Piku Rules
...
2. Commit the change
3. Piku will sync only that specific file
...
To keep a rule/guidance file in your repository but stop Piku from turning it into Piku Rules, add `@kody-ignore` anywhere in the file:

1. Add `@kody-ignore` anywhere in the rule file (case-insensitive)
2. Commit the change

On the next sync Piku will **skip** that file **and remove any Piku Rules previously created from it**. The rules come back automatically if you later remove the `@kody-ignore` marker (or add `@kody-sync`). Use this to keep house-style docs like a `CLAUDE.md` or `AGENTS.md` in the repo without them becoming enforced review rules.

<Warning>
If you manually edit a Piku Rule that was created from an IDE rule file, your changes will be overwritten the next time the corresponding file is modified and synced.
</Warning>
