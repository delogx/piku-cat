...
description: "How a Piku review runs end to end: when it triggers, when it skips, what you get back, and what each status emoji means."
---

## Overview

Piku reviews your PRs automatically or on demand, adds clear suggestions, and helps your team decide when to merge. Below is the simple, user-facing view of how it works.
...
Piku uses emoji reactions to show the live status of your code review directly in your Git provider. No guessing whether she’s working, done, or ran into a problem. She always replaces the previous emoji with the latest status (for example: 🚀 → 🎉, 👀, 👎 or 😕).

<Note>
  **Native reactions** work on GitHub, GitLab, and Forgejo.

  Azure DevOps and Bitbucket have no reaction API, so Piku falls back to posting the status
...
- 🚀 Processing — Piku is working on your code:
  - Analyzing files
...
- 👎 No license — The user is not licensed:
  - Piku cannot run without a valid license
...
- Manual review (comment `@kody start-review`): reaction on your comment where you invoked Piku
...
- Normal—Piku replaces previous status (🚀 → 🎉/👀/😕)
...
- Review filters: minimum severity and whether they apply to Piku Rules
