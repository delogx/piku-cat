...
description: "Generate the Bitbucket Cloud API Token Piku needs for code review, with the exact permissions to grant."
---

## Introduction

To enable Piku to perform automatic code reviews on Bitbucket Cloud, you need to generate an **API Token**. This token allows Piku to access your repositories, pull requests, webhooks, issues, pipelines etc., with the specific permissions listed below.
...
## Required Scopes for Piku

The token must include **all** of the following scopes for full functionality with Piku:
...
- Once the token is generated with all the required scopes, paste it into the **Piku integration / automation setup screen** when enabling the Bitbucket integration.
- Make sure the token has `admin:repository:bitbucket` (or equivalent) if Piku needs to configure webhooks or listen for pull request events.
