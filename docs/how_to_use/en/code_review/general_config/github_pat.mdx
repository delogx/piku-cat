...
description: "Generate the GitHub Fine-Grained PAT Piku needs for code review, including how to unblock access to your organization's repositories."
---

## Introduction to FGPAT

To enable Piku to perform automatic Code Reviews, you need to generate a Fine-Grained Personal Access Token (FGPAT) in GitHub. This token allows Piku to securely access your repositories and perform code analysis in a controlled manner.
...
1. Under **Fine-Grained Personal Access Tokens**, select **"Allow access via fine-grained personal access tokens"** to enable secure access for Piku.
...
6. **Finalize and save the token**:
   ...
   - Make sure to copy the generated token and store it securely, as you’ll need it to configure Piku.

## Adding the Token to Piku

After generating the token, paste it in the Piku configuration screen for automation setup. The modal will open automatically as soon as you attempt to enable automation.
