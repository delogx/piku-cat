...
description: "Direct Piku's attention to a specific area of a pull request with a one-off review directive, via PR comment or CLI"
---

A **review directive** lets you tell Piku where to focus its deepest analysis for a single review. Instead of changing any configuration, you attach a short instruction when you trigger the review — for example, "focus on the auth and session logic" — and Piku spends its hardest analysis on the changed code that matches.

<Note>
  A directive is **ephemeral**: it applies to that one review run only. It is not saved to your settings and does not affect future reviews. To change Piku's behavior permanently, use [Custom Prompts](/how_to_use/en/code_review/configs/custom_prompts) or [Piku Rules](/how_to_use/en/code_review/configs/kody_rules) instead.
</Note>

## When to Use It

- A PR touches many files but you care most about one risky area (auth, payments, a migration).
- You want Piku to trace the callers and callees of a specific change and challenge it harder.
...
<Warning>
  A directive sets **priority, not a filter**. Piku prioritizes the focus area, but it still reports any concrete bug, security, or performance issue it notices **elsewhere** in the diff. It never suppresses findings outside the focus, and it never approves the rest of the PR unseen.
</Warning>
...
<Tabs>
  <Tab title="PR / MR Comment">
    ...
    <Note>
      `--force` re-runs the review even when Piku would normally skip it ("no new commits since the last review"). Use it — with or without a focus directive — to get a fresh pass after fixing whatever made the previous run fail, or to apply updated settings to an already-reviewed PR.
    </Note>
...
A focus directive tells Piku *where* to look hardest. `--heavy` tells it to look *longer*: the
...
- **Untrusted by design:** because anyone who can comment on the PR can supply a directive, the text is sanitized before use (control characters and angle brackets are stripped). This does not change your focus — it only prevents the text from tampering with Piku's internal prompt.
