...
description: "How piku-cat pricing works — Community, Teams, and Enterprise, what the free trial includes, and why reviews are unlimited on every plan."
---
...
<Note>
  **Reviews are unlimited on every plan** — they run on your own AI key (BYOK).
  Plans differ by **features**, not by how many PRs Piku can review. Every plan
...
<CardGroup cols={2}>
  <Card title="14 days of Team features" icon="calendar">
    Cockpit, unlimited Piku Rules and plugins, priority queue, and more.
  </Card>
  <Card title="Your first 5 PR reviews on us" icon="gift">
    We cover the AI tokens, so you can try Piku with **zero setup — no API key
    needed.** (Earn a few more by completing onboarding steps.)
  </Card>
</CardGroup>

**To keep reviewing past the 5 trial reviews — or to go unlimited right away — [connect your AI key](/how_to_use/en/byok).** Reviews then run on your key: unlimited, on any plan, even after the 14 days end. Trial reviews run on a model piku-cat picks for you; with your own key you can run frontier models for the best quality.

If trial reviews run out before you connect a key, Piku pauses reviews and posts a comment with a one-click link to add your key.
...
| **Piku Learnings & Memory** | ✓ | ✓ | ✓ |
| **Priority queue** | — | ✓ | ✓ |
| **piku-cat Cockpit (metrics)** | — | ✓ | ✓ |
| **SSO / SAML** | — | — | ✓ |
| **RBAC + audit logs** | — | — | ✓ |
| **SOC 2 compliant** | — | — | ✓ |
...
piku-cat does **not** sell LLM tokens by default. Community, Teams, and Enterprise all run on **Bring Your Own Key** — you connect your own provider (OpenAI, Anthropic, Gemini, or any OpenAI-compatible), and tokens are billed directly by that provider. piku-cat never marks up tokens.

| Plan | What you pay piku-cat | Tokens |
| --- | --- | --- |
...
In addition to your provider's token cost (no piku-cat markup).

<CardGroup cols={2}>
  <Card title="What the $10/dev covers" icon="box">
    - Managed cloud hosting + automatic updates
    - Priority queue for Piku Agents
    - piku-cat Cockpit (engineering metrics)
    - Unlimited Piku Rules & plugins/MCPs
    - Email + Discord support
  </Card>
  <Card title="What tokens cover" icon="coins">
    - Every LLM call Piku makes during a review
...
Custom-priced via contract. Like every plan, Enterprise is **BYOK** — piku-cat bills only the per-seat fee and you pay your LLM provider directly.

Enterprise adds SSO / SAML, RBAC with audit logs and analytics, a dedicated instance, custom SLAs, up to **5 hours/month** of dedicated support, and a private Discord channel. piku-cat is **SOC 2 compliant**.
...
We subsidize piku-cat with **30–100% discounts** for teams that move the industry forward or couldn't otherwise afford it:
