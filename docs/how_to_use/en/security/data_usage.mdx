...
There is no single fixed model. **piku-cat runs on Bring Your Own Key (BYOK) by default on every
...
| **Trial / no key connected** | piku-cat, for a limited number of reviews. | A coding-specialized model we pick, with a smaller open-weights model as fallback. |
| **Enterprise with a piku-cat-managed key** | piku-cat, under your Enterprise agreement. | Frontier general-purpose models, selected by us. |

### Supported provider classes

Under BYOK you can point piku-cat at:
...
<Note>
  **A BYOK review never silently falls back to a piku-cat-managed model.** If your configured model
...
## What piku-cat does with your code
...
- Nothing is retained for training by piku-cat.
...
contract with piku-cat — it's governed by yours with the provider.**
...
- Retention windows (for abuse monitoring, for example) are set by the provider, not by piku-cat.
...
<Warning>
  Because you choose the provider, **piku-cat cannot make data-handling promises on their behalf.**
  If your compliance posture depends on specific guarantees, confirm them with your provider
  before pointing piku-cat at it.
</Warning>

## Security measures on the piku-cat side
...
- **Self-hosting**: run the whole stack in your own infrastructure if you don't want code
  leaving your network at all. See [Deploy piku-cat](/how_to_deploy/en/deploy_kodus/generic_vm).

## Self-hosted telemetry

If you run piku-cat self-hosted, each instance sends one anonymous daily
