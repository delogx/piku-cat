...
description: "Configura tus propias claves de API para máxima flexibilidad y control de costos. Disponible en todos los planes de piku-cat."
---

BYOK (Bring Your Own Key) es la **forma predeterminada en que piku-cat usa LLMs en todos los planes** — Community, Teams y Enterprise. Conectas tu propia cuenta de proveedor, eliges un modelo, pagas solo por lo que usas y monitoreas los costos directamente en el panel de tu proveedor. piku-cat nunca aplica márgenes sobre los tokens y nunca ve tu clave en texto plano.
...
>
  BYOK es gratuito en Community, incluido en Teams ($10/desarrollador activo/mes sobre tu gasto en tokens) y una de dos opciones en Enterprise (la otra es una clave de API administrada por piku-cat).
</Card>
...
<Steps>
  ...
  <Step title="Agregar un modelo de respaldo (recomendado)">
    Una vez configurado el modelo principal, aparece una sección **Modelo de respaldo**. Si tu proveedor principal alcanza límites de tasa o deja de estar disponible, piku-cat conmuta automáticamente.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="GLM 5.1 (Z.ai)">
    ...
    <Warning>
      Las claves del GLM Coding Plan **solo** funcionan en `/api/coding/paas/v4`. Los niveles Lite y Pro suelen estar limitados a **1 solicitud concurrente** — piku-cat rellena previamente `maxConcurrentRequests=1` cuando eliges este plan. Auméntalo en la configuración avanzada si estás en el nivel Max (hasta 30).
    </Warning>
  </Tab>
  ...
  <Tab title="Kimi K2.6 (Moonshot AI)">
    ...
    <Info>
      El Kimi Code Plan está documentado con un tope de 30 solicitudes concurrentes. piku-cat rellena previamente `maxConcurrentRequests=30` cuando eliges ese plan.
    </Info>
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Elegir o escribir el ID del modelo">
    Si piku-cat puede listar los modelos del proveedor, obtienes un menú desplegable. De lo contrario (por ejemplo, auto-alojado o cuando las claves de la plataforma no están configuradas), escribe el ID exacto del modelo manualmente.
  </Step>
...
<Tabs>
  <Tab title="OpenAI">
    ...
    **Obtener una clave de API:**
    ...
    2. Crea una nueva clave para piku-cat
  ...
  <Tab title="Google Vertex AI">
    ...
    **Cómo configurarlo:**
    ...
    2. Pega el **contenido del archivo JSON de tu service account** en el campo Service Account JSON (base64 también funciona). piku-cat extrae el `project_id` del JSON automáticamente.
  ...
  <Tab title="Anthropic-compatible">
    ...
    **Cómo configurarlo:**
    ...
    <Note>
      Pega la base URL en el formato que documente tu proveedor — con o sin `/v1` al final (p. ej. `https://api.kimi.com/coding` o `https://api.kimi.com/coding/v1`). piku-cat la normaliza internamente, porque las dos vías del SDK de Anthropic no coinciden en dónde va el `/v1`.
    </Note>
  </Tab>
</Tabs>
...
Cuando eliges Low / Medium / High, piku-cat traduce el nivel al formato nativo de cada proveedor automáticamente:
...
<Note>
  Kimi y GLM actualmente exponen el razonamiento como un único indicador on/off. Elegir Low, Medium o High emite el mismo payload (thinking habilitado). Cuando sus APIs añadan granularidad de nivel, piku-cat empezará a reenviarla.
</Note>

### Sobrescritura con JSON personalizado

Elegir **Custom** en el interruptor de Thinking revela un área de texto JSON. Pega directamente las opciones del proveedor — **piku-cat las envuelve automáticamente bajo el namespace del proveedor activo**. No necesitas conocer las reglas de enrutamiento del Vercel AI SDK.
...
- El proveedor lanza un nuevo campo que piku-cat aún no ha envuelto
...
Si tu JSON ya contiene una clave de namespace conocida en el nivel superior (`anthropic`, `google`, `openai`, `openrouter`, `openaiCompatible`), piku-cat lo deja intacto. Útil si quieres mezclar múltiples namespaces de proveedores o ser explícito:
...
Internamente, estos son los mapeos de namespace que piku-cat usa:
...
- **Solo JSON válido.** Las comas faltantes o sobrantes rompen el análisis y piku-cat ignora la sobrescritura.
...
- **Proveedor desconocido = sin envoltura.** Si tu proveedor de BYOK no está en la tabla de namespaces anterior, piku-cat pasa el JSON tal cual. Raro — solo aplica si configuras un proveedor que piku-cat no reconoce.
...
Internamente, piku-cat emite esto en la llamada del Vercel AI SDK:
...
El campo `maxConcurrentRequests` (bajo **Configuración avanzada**) limita cuántas solicitudes simultáneas envía piku-cat a tu proveedor en paralelo. La mayoría de las veces, el valor predeterminado es suficiente — pero los planes de suscripción con topes de concurrencia estrictos necesitan configurarlo explícitamente.

### Valores predeterminados que piku-cat rellena
...
| **Anthropic / OpenAI / Google / OpenRouter** | _(vacío)_ | Los proveedores aplican sus propios TPM/RPM; piku-cat no lo limita. |
...
<Note>
  **Interacción con el respaldo.** Cuando el principal recibe un 429 y piku-cat conmuta al modelo de respaldo, se aplica el `maxConcurrentRequests` del propio respaldo. Configurar un respaldo generoso en un proveedor distinto es una buena forma de absorber ráfagas cuando tu principal está en una suscripción ajustada.
</Note>
...
<CardGroup cols={2}>
  <Card title="Claves dedicadas" icon="shield-check">
    Crea claves de API separadas para piku-cat. Facilita la auditoría de uso y la rotación de claves.
  </Card>
  ...
  <Card title="Almacenamiento seguro" icon="lock">
    Nunca confirmes claves en repositorios. piku-cat las almacena cifradas en reposo y en tránsito.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Variables de entorno de alojamiento autónomo no aparecen">
    - Si piku-cat está configurado mediante `.env` (Modo Fijo auto-alojado), la pantalla de BYOK muestra un banner informativo azul con el proveedor/modelo activo — la clave nunca se muestra por seguridad.
...
<AccordionGroup>
  ...
  <Accordion title="¿Cómo funciona el sistema principal/respaldo?">
    El modelo principal gestiona todas las revisiones por defecto. Si falla (límite de tasa, 5xx, tiempo de espera), piku-cat reintenta una vez en el respaldo. Solo pagas por el proveedor que efectivamente procesó la revisión.
  </Accordion>
