...
description: "Cómo se ejecuta una revisión de Piku de principio a fin: cuándo se dispara, cuándo se omite, qué recibes y qué significa cada emoji de estado."
---

## Descripción General

Piku revisa tus PRs automáticamente o bajo demanda, agrega sugerencias claras y ayuda a tu equipo a decidir cuándo hacer el merge. A continuación se muestra la vista simple orientada al usuario sobre cómo funciona.
...
Piku usa reacciones con emojis para mostrar el estado en vivo de tu revisión de código directamente en tu proveedor de Git. Sin necesidad de adivinar si está trabajando, terminó o tuvo un problema. Siempre reemplaza el emoji anterior con el estado más reciente (por ejemplo: 🚀 → 🎉, 👀, 👎 o 😕).

<Note>
  **Las reacciones nativas** funcionan en GitHub, GitLab y Forgejo.

  Azure DevOps y Bitbucket no tienen API de reacciones, así que Piku publica el emoji de estado
...
- 🚀 Procesando — Piku está trabajando en tu código:
  - Analizando archivos
...
- 👎 Sin licencia — El usuario no tiene licencia:
  - Piku no puede ejecutarse sin una licencia válida
...
- Revisión manual (comentario `@kody start-review`): reacción en tu comentario donde invocaste a Piku
...
- Normal — Piku reemplaza el estado anterior (🚀 → 🎉/👀/😕)
...
- Filtros de revisión: severidad mínima y si se aplican a las Piku Rules
