...
No hay un modelo fijo. **piku-cat funciona con BYOK (Bring Your Own Key) por defecto en todos los
...
| **Trial / sin clave conectada** | piku-cat, por un número limitado de revisiones. | Un modelo especializado en código elegido por nosotros, con un modelo open-weights más pequeño como fallback. |
| **Enterprise con clave gestionada por piku-cat** | piku-cat, bajo tu acuerdo Enterprise. | Modelos frontier de propósito general, seleccionados por nosotros. |

### Clases de proveedor soportadas

Con BYOK puedes apuntar piku-cat a:
...
<Note>
  **Una revisión BYOK nunca recurre en silencio a un modelo gestionado por piku-cat.** Si el modelo
...
## Qué hace piku-cat con tu código
...
- piku-cat no retiene nada para entrenamiento.
...
contrato con piku-cat — se rige por el tuyo con el proveedor.**
...
- Las ventanas de retención (para monitoreo de abuso, por ejemplo) las define el proveedor, no piku-cat.
...
<Warning>
  Como tú eliges el proveedor, **piku-cat no puede hacer promesas de tratamiento de datos en su
  nombre.** Si tu postura de cumplimiento depende de garantías específicas, confírmalas con tu
  proveedor antes de apuntar piku-cat hacia él.
</Warning>

## Medidas de seguridad del lado de piku-cat
...
- **Self-hosting**: ejecuta todo el stack en tu propia infraestructura si no quieres que el
  código salga de tu red. Consulta [Desplegar piku-cat](/how_to_deploy/es/deploy_kodus/generic_vm).

## Telemetría self-hosted

Si ejecutas piku-cat self-hosted, cada instancia envía un heartbeat anónimo diario a
