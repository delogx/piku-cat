...
| `--github-pat <token>` | —      | プライベートリポジトリのクローンに使う GitHub トークン（`read:repo`）。トライアルユーザーのみ。`KODUS_GITHUB_PAT` でも設定可能。メモリ上のみで保持され、永続化されません。 |
...
piku-cat によってインストールされた pre-push フックを削除します。
...
piku-cat に保存されたリポジトリ設定を確認・更新します。チームキー認証が必要です。現在のリポジトリを対象にするには `.` を使用します。
...
```bash
kodus config repo add                                   # 現在のリポジトリを piku-cat に追加
...
kodus config repo open                                  # piku-cat ダッシュボードを開く
...
| サブコマンド                                       | 説明                                                                        |
| -------------------------------------------------- | --------------------------------------------------------------------------- |
| `add [repository]`                                 | piku-cat にリポジトリを追加（ショートカット: `kodus config -r [repository]`）  |
| `list`                                             | piku-cat で設定されたリポジトリを一覧表示                                      |
| `show [repository]`                                | リポジトリ設定を表示                                                        |
| `setup [repository]`                               | ガイド付きリポジトリセットアップウィザードを実行                            |
| `set [repository] <key> <value>`                   | リポジトリ設定を直接設定                                                    |
| `open [repository]`                                | 高度な設定のための piku-cat ダッシュボードを開く                               |
...
Piku Rules を作成、更新、表示します — piku-cat がレビュー中に適用する構造化ルール。
...
認証モード、組織、リポジトリ、フック、決定、バンドルされたスキル、CLI バージョンなど、統合された piku-cat ステータスを表示します。
...
<Note>
  `kodus review` を実行する前に変更セットをざっと確認するのに便利です。同じビューアーが
  インタラクティブなレビュー体験を支えています。従来のリストがよい場合は `kodus review` に
  `--no-hunk` を渡してください。
</Note>
...
ブラウザで piku-cat の価格/アップグレードページを開きます。
...
| `style`                  | コードスタイルの問題                   |
| `documentation`          | ドキュメントの不足（`kodus pr suggestions` の `--category` で指定可能） |
