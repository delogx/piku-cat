---
title: "セルフホストセットアップ"
description: "Kodus CLI を自己ホスト型 piku-cat インスタンスに接続します。"
---

独自のインフラで piku-cat を実行している場合、CLI は同じように動作します — API に向けるだけです。

## インスタンスへの接続

<Steps>
  <Step title="API URL を設定">
    piku-cat API の場所を CLI に伝えます。方法を1つ選択してください:
  ...
  <Step title="認証">
    <Tabs>
      <Tab title="チームキー（推奨）">
        セルフホスト型 piku-cat ダッシュボードからチームキーを生成して設定:
