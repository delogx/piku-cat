...
description: "Configure suas próprias chaves de API para máxima flexibilidade e controle de custos. Disponível em todos os planos do piku-cat."
---

BYOK (Bring Your Own Key) é a **forma padrão como o piku-cat usa LLMs em todos os planos** — Community, Teams e Enterprise. Você conecta sua própria conta de provedor, escolhe um modelo, paga apenas pelo que usa e monitora custos diretamente no painel do seu provedor. O piku-cat nunca cobra margem sobre tokens e nunca vê sua chave em texto simples.
...
>
  O BYOK é gratuito no Community, incluído no Teams ($10/dev ativo/mês além do seu gasto com tokens) e uma das duas opções no Enterprise (a outra sendo uma chave de API gerenciada pelo piku-cat).
</Card>
...
<Steps>
  ...
  <Step title="Adicionar um Fallback (recomendado)">
    Uma vez que o modelo Main esteja configurado, uma seção **modelo Fallback** aparece. Se seu provedor principal atingir limites de taxa ou ficar indisponível, o piku-cat faz fallback automaticamente.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="GLM 5.1 (Z.ai)">
    ...
    <Warning>
      Chaves do GLM Coding Plan funcionam **apenas** em `/api/coding/paas/v4`. Os tiers Lite e Pro geralmente são limitados a **1 requisição simultânea** — o piku-cat pré-preenche `maxConcurrentRequests=1` quando você escolhe esse plano. Aumente em Advanced settings se estiver no tier Max (até 30).
    </Warning>
  </Tab>
  ...
  <Tab title="Kimi K2.6 (Moonshot AI)">
    ...
    <Info>
      O Kimi Code Plan está documentado com um limite de 30 requisições simultâneas. O piku-cat pré-preenche `maxConcurrentRequests=30` quando você escolhe esse plano.
    </Info>
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Escolha ou digite o Model ID">
    Se o piku-cat conseguir listar modelos do provedor, você recebe um dropdown. Caso contrário (ex.: self-hosted ou quando chaves de plataforma não estão configuradas), digite o Model ID exato manualmente.
  </Step>
...
<Tabs>
  <Tab title="OpenAI">
    ...
    **Obter uma chave de API:**
    ...
    2. Crie uma nova chave para o piku-cat
  ...
  <Tab title="Google Vertex AI">
    ...
    **Como configurar:**
    ...
    2. Cole o **conteúdo do arquivo JSON da sua service account** no campo Service Account JSON (base64 também funciona). O piku-cat extrai o `project_id` do JSON automaticamente.
  ...
  <Tab title="Anthropic-compatible">
    ...
    **Como configurar:**
    ...
    <Note>
      Cole a base URL no formato que seu provedor documenta — com ou sem `/v1` no final (ex.: `https://api.kimi.com/coding` ou `https://api.kimi.com/coding/v1`). O piku-cat normaliza internamente, porque os dois caminhos do SDK da Anthropic discordam sobre onde o `/v1` entra.
    </Note>
  </Tab>
</Tabs>
...
Quando você escolhe Low / Medium / High, o piku-cat traduz o nível para o formato nativo de cada provedor automaticamente:
...
<Note>
  Kimi e GLM atualmente expõem reasoning como um flag único on/off. Escolher Low, Medium ou High emite o mesmo payload (thinking habilitado). Quando suas APIs adicionarem granularidade de nível, o piku-cat começará a encaminhá-la.
</Note>

### Sobrescrita via JSON customizado

Escolher **Custom** no toggle Thinking revela uma textarea de JSON. Cole as opções do provedor diretamente — **o piku-cat faz o auto-wrap sob o namespace do provedor ativo**. Você não precisa conhecer as regras de roteamento do Vercel AI SDK.
...
- O provedor lança um novo campo que o piku-cat ainda não envolveu
...
Se seu JSON já contém uma chave de namespace conhecida no nível superior (`anthropic`, `google`, `openai`, `openrouter`, `openaiCompatible`), o piku-cat deixa intocado. Útil quando você quer misturar múltiplos namespaces de provedor ou ser explícito:
...
Nos bastidores, estes são os mapeamentos de namespace que o piku-cat usa:
...
- **Apenas JSON válido.** Vírgulas faltando ou vírgulas finais quebram o parse e o piku-cat ignora a sobrescrita.
...
- **Provedor desconhecido = sem wrap.** Se seu provedor BYOK não está na tabela de namespace acima, o piku-cat passa o JSON como está. Raro — só se aplica se você configurar um provedor que o piku-cat não reconhece.
...
Nos bastidores, o piku-cat emite isto na chamada ao Vercel AI SDK:
...
O campo `maxConcurrentRequests` (em **Advanced settings**) limita quantas requisições em voo o piku-cat envia ao seu provedor em paralelo. Na maioria das vezes, o padrão é suficiente — mas planos de assinatura com limites estritos de simultaneidade precisam defini-lo explicitamente.

### Padrões que o piku-cat pré-preenche
...
| **Anthropic / OpenAI / Google / OpenRouter** | _(vazio)_ | Provedores aplicam seus próprios TPM/RPM; o piku-cat não limita. |
...
<Note>
  **Interação com Fallback.** Quando o Main bate em 429 e o piku-cat faz failover para o modelo Fallback, o `maxConcurrentRequests` do próprio Fallback se aplica. Definir um Fallback generoso em um provedor diferente é uma boa forma de absorver picos quando seu Main está em uma assinatura apertada.
</Note>
...
<CardGroup cols={2}>
  <Card title="Chaves Dedicadas" icon="shield-check">
    Crie chaves de API separadas para o piku-cat. Facilita auditoria de uso e rotação de chaves.
  </Card>
  ...
  <Card title="Armazenamento Seguro" icon="lock">
    Nunca faça commit de chaves em repositórios. O piku-cat armazena as chaves criptografadas em repouso e em trânsito.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Variáveis de ambiente self-hosted não aparecem">
    - Se o piku-cat está configurado via `.env` (Fixed Mode self-hosted), a tela de BYOK exibe um banner informativo azul com o provedor/modelo ativo — a chave nunca é exibida por segurança.
...
<AccordionGroup>
  ...
  <Accordion title="Como funciona o sistema primário/fallback?">
    O Main lida com todas as revisões por padrão. Se falhar (rate limit, 5xx, timeout), o piku-cat tenta novamente uma vez no Fallback. Você paga apenas pelo provedor que efetivamente processou a revisão.
  </Accordion>
