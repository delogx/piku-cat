...
| `--github-pat <token>` | —     | Token do GitHub (`read:repo`) usado para clonar repositórios privados. Apenas usuários em trial. Também via `KODUS_GITHUB_PAT`. Fica só em memória, nunca é persistido. |
...
Gerencia autenticação com a API do piku-cat.
...
Remove o hook pre-push instalado pelo piku-cat.
...
Inspeciona e atualiza configurações de repositório armazenadas no piku-cat. Requer autenticação por chave de equipe. Use `.` para referenciar o repositório atual.
...
```bash
kodus config repo add                                   # Adicionar repositório atual ao piku-cat
...
kodus config repo open                                  # Abrir o painel do piku-cat
...
| Subcomando                                         | Descrição                                                                   |
| -------------------------------------------------- | --------------------------------------------------------------------------- |
| `add [repository]`                                 | Adicionar um repositório ao piku-cat (atalho: `kodus config -r [repository]`) |
| `list`                                             | Listar repositórios configurados no piku-cat                                   |
...
| `open [repository]`                                | Abrir o painel do piku-cat para configurações avançadas                        |
...
Cria, atualiza e visualiza Piku Rules — regras estruturadas que o piku-cat aplica durante as revisões.
...
| Subcomando | Descrição                    |
| ---------- | ---------------------------- |
| `create`   | Criar uma nova Piku Rule     |
| `update`   | Atualizar uma regra existente |
| `view`     | Visualizar Piku Rules        |
...
Exibe um status consolidado do piku-cat: modo de autenticação, organização, repositório, hook, skills incluídas e versão do CLI.
...
<Note>
  Útil para dar uma olhada no changeset antes de rodar o `kodus review` nele. O mesmo
  visualizador sustenta a experiência de revisão interativa; passe `--no-hunk` ao `kodus review`
...
Abre a página de preços/upgrade do piku-cat no seu navegador.
...
| `documentation`          | Lacunas de documentação (aceito por `--category` em `kodus pr suggestions`) |
