...
description: "Meça o quão eficazes são as revisões do piku-cat — e aja com base nisso"
---

## O que é a aba piku-cat Review

A aba **piku-cat Review** responde a uma pergunta: *o seu time está agindo com base no que o piku-cat diz?*

Diferente da aba Productivity (que mede métricas gerais de entrega, como frequência de deploy e tempo de ciclo de PR), a piku-cat Review trata do próprio piku-cat — quantas sugestões são implementadas, quais categorias e regras pegam ou são ignoradas, e onde o time empurra de volta.
...
| **Suggestions sent** | Quantas sugestões o piku-cat entregou (como comentários de PR) no período |
...
- Apenas sugestões **entregues** contam (aquelas de fato postadas como comentários de PR). Rascunhos que o piku-cat filtrou antes de comentar nunca são contabilizados.
...
<Warning>
  **O seletor "All / piku-cat only" no gráfico de severidade importa.** Uma Piku Rule carrega a severidade que *você* definiu na regra, não uma avaliação de risco feita pelo piku-cat. Misturar as duas distorce a leitura de calibração — por exemplo, um monte de Piku Rules medium com alta adoção pode fazer o piku-cat parecer que subestima medium. Mude para **piku-cat only** para ver a calibração de severidade do próprio piku-cat. Barras construídas sobre pouquíssimas sugestões aparecem esmaecidas e marcadas com `*` — um 0% ou 100% vindo de um punhado de sugestões não é um sinal real.
</Warning>

## Feedback negativo

O feedback vem das reações 👍 / 👎 nos comentários de sugestão do piku-cat.
...
Uma tabela por repositório: PRs revisados, sugestões enviadas, implementation rate, 👍/👎, e a **categoria mais fraca** (a categoria com a menor implementation rate naquele repositório, dado um tamanho mínimo de amostra). Mostra onde o piku-cat está pegando versus sendo ignorado.

Clicar em uma linha foca todo o cockpit naquele repositório (o mesmo que selecioná-lo no filtro de repositório).

## Piku Rules — saúde

Como cada Piku Rule está performando no período: gatilhos, implementation rate, 👍/👎, e um status. Apenas regras ativas aparecem — regras deletadas ou inativas são excluídas, já que você não pode agir sobre elas.
...
Filtros: repositório, categoria, severidade, status de implementação, Piku Rule e busca por texto livre. Cada linha expande para mostrar o código existente vs. o sugerido e um link para o comentário do PR.
