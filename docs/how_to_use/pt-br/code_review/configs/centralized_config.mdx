...
description: "Gerencie toda a configuração de code review e as Piku Rules como código em um único repositório. As alterações são sincronizadas automaticamente após cada merge; o rollout é controlado pela UI ou pelo CLI."
---

A Configuração Centralizada permite manter todas as configurações de code review e todas as Piku Rules em **um único repositório**, em vez de editá-las por repositório na interface web. Cada alteração passa por um pull request nesse repositório, fornecendo histórico de versões, revisão e rollback da sua configuração de revisão.
...
- Piku Rules consistentes (regras de revisão e memórias) em toda a organização.
- Alterações de configuração via CLI ou CI.

## Como Funciona

1. Escolha um repositório para armazenar a configuração centralizada. Ele precisa ser um dos repositórios selecionados no piku-cat.
2. Habilite a Configuração Centralizada na UI e aponte para esse repositório.
3. Escolha um **modo de sincronização inicial**:
   - `pr` — piku-cat abre o PR de inicialização com suas configurações e regras efetivas atuais.
   - `manual` — piku-cat gera os arquivos; sua equipe faz o download e abre o PR.
4. Após o primeiro merge, todo merge futuro no repositório centralizado propaga automaticamente as alterações para o piku-cat.

O repositório centralizado espelha a hierarquia de escopos do piku-cat (Global → Repositório → Diretório) por meio do seu layout de pastas.

## Estrutura do Repositório

O repositório centralizado usa caminhos de pastas para mapear arquivos a escopos. A estrutura cobre tanto as configurações de revisão (`kodus-config.yml`) quanto as Piku Rules (`.kody-rules/`).
...
│
├── backend-api/                          ← corresponde a um repositório piku-cat
...
- Uma pasta de nível superior cujo nome corresponde a um repositório piku-cat (ex.: `backend-api/`) escopa os arquivos para aquele repositório.
...
## Arquivos de Piku Rules

Cada regra é um único arquivo YAML em `.kody-rules/review/` (regras padrão) ou `.kody-rules/memories/` (memórias). O nome do arquivo é arbitrário — piku-cat identifica as regras pelo campo `title` e pelo caminho canônico do arquivo armazenado com a regra.
...
Veja a [Visão Geral das Piku Rules](/how_to_use/pt-br/code_review/configs/kody_rules) para a referência completa de campos e [Herança de Regras](/how_to_use/pt-br/code_review/configs/rules_inheritance) para entender como `inheritance.inheritable`, `exclude` e `include` se comportam.
...
- **Piku Rules e Mensagens de PR** permanecem editáveis na interface web. Salvar uma edição não modifica a regra diretamente — piku-cat abre um pull request no repositório centralizado com a alteração proposta. Uma vez que o PR é mesclado, a alteração entra em vigor.
...
- Na hora da revisão, o piku-cat carrega **todas as regras ativas** do banco de dados independentemente da origem. Uma regra que existe só na UI é aplicada exatamente como uma regra que existe só no repo.
...
| Status | Significado |
|---|---|
| `SYNCED` | A regra no piku-cat corresponde ao arquivo no repositório centralizado. |
...
<Tabs>
  <Tab title="PR (padrão)">
    piku-cat abre o pull request de inicialização automaticamente com suas
    configurações e regras efetivas atuais.
  </Tab>
  <Tab title="Manual">
    piku-cat gera os arquivos mas não abre um PR. Use o bundle de download
...
As regras e configurações que foram sincronizadas permanecem no piku-cat — elas não são excluídas ao desabilitar.
...
- Pelo menos um repositório selecionado no piku-cat.
...
- [Visão Geral das Piku Rules](/how_to_use/pt-br/code_review/configs/kody_rules)
