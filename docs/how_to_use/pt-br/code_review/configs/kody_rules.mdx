---
title: "Visão Geral"
description: "Piku Rules são diretrizes personalizáveis que sua equipe configura para aplicar automaticamente qualidade de código, consistência, segurança e manutenibilidade."
---
...
## Dois tipos na interface de Piku Rules

As Piku Rules são organizadas em duas categorias, gerenciadas por meio de uma interface com abas nas suas configurações:
...
<Tabs>
  ...
  <Tab title="Memórias">
    Instruções contextuais persistentes que são injetadas em todos os prompts e conversas. As Memórias permitem que Piku aprenda e retenha como seu codebase funciona para que futuras revisões e sugestões sejam mais relevantes.

    - Injetadas como **contexto de alta prioridade** em revisões de código, análise entre arquivos e conversas
    - Criadas via **conversa com Piku** ou **manualmente na interface**
...
- Piku identifica referências a arquivos automaticamente quando você salva uma regra
...
- O conteúdo dos arquivos é injetado no contexto da regra, permitindo que Piku compare, valide ou aplique padrões
...
- Navegue até Descoberta de Regras no seu painel piku-cat.
...
- **Melhores sugestões futuras**: As Memórias ajudam o Piku a parar de repetir as mesmas suposições erradas sobre seu repositório
- **Deduplicação inteligente**: Quando uma nova memória é criada, Piku usa um mecanismo de resolução baseado em LLM para determinar se deve criá-la, ignorá-la (se duplicada) ou atualizar uma memória existente
...
A forma mais natural de criar memórias é por meio de conversa com Piku nos comentários do PR. Piku detecta intenções explícitas e implícitas de salvar convenções:

**Explícito** — pedindo diretamente para Piku lembrar:
...
**Implícito** — declarando uma preferência ou convenção que Piku captura:
...
<Info>
  Piku NÃO criará memórias para instruções transitórias (ex.: "corrija isso agora"), conversas de debug, perguntas, declarações vagas ou solicitações explicitamente limitadas a uma única tarefa ou PR.
</Info>

Quando uma memória é criada ou atualizada, Piku responde com uma confirmação e fornece um **link direto** para visualizar a memória na interface.

#### Via Interface

Você também pode criar memórias manualmente:

1. Acesse **Configurações de Code Review** → **Repositório** → **Piku Rules**
...
1. Acesse **Configurações de Code Review** → **Repositório** → **Piku Rules** → aba **Memórias**
