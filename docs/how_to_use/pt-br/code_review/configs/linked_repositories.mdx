...
Com o backend vinculado, o Piku verifica o outro lado desse contrato e comenta no PR:
...
<Info>
- Só podem ser vinculados repositórios **já conectados à mesma organização piku-cat** (o Piku reaproveita sua integração de Git existente — sem tokens extras).
- Até **3 repositórios vinculados** por repositório.
- O acesso é somente leitura. O Piku nunca faz push, comenta nem modifica um repositório vinculado.
...
Uma dica em texto livre que diz ao agente de revisão **para que serve esse vínculo e onde olhar**. O Piku funciona sem ela, mas uma boa instrução deixa a passagem entre repositórios mais rápida e precisa — é a diferença entre o agente explorar o repositório vinculado do zero e ir direto ao código relevante.
...
Controla qual branch ("ref") do repositório vinculado o Piku lê. **A maioria dos times deixa vazio**: o Piku então lê a branch com o mesmo nome da branch do seu PR quando ela existe (para que mudanças coordenadas entre repositórios se enxerguem antes do merge), e cai na branch padrão caso contrário.
