...
description: "Como uma revisão do Piku roda de ponta a ponta: quando dispara, quando pula, o que você recebe de volta e o que cada emoji de status significa."
---

## Visão Geral

Piku revisa seus PRs automaticamente ou sob demanda, adiciona sugestões claras e ajuda sua equipe a decidir quando fazer o merge. Abaixo está a visão simples e orientada ao usuário de como funciona.
...
Piku usa reações com emoji para mostrar o status em tempo real do seu code review diretamente no seu provedor Git. Sem precisar adivinhar se ela está trabalhando, concluída ou encontrou um problema. Ela sempre substitui o emoji anterior pelo status mais recente (por exemplo: 🚀 → 🎉, 👀, 👎 ou 😕).

<Note>
  **Reações nativas** funcionam no GitHub, GitLab e Forgejo.

  Azure DevOps e Bitbucket não têm API de reações, então o Piku publica o emoji de status
...
- 🚀 Processando — Piku está trabalhando no seu código:
  - Analisando arquivos
...
- 👎 Sem licença — O usuário não está licenciado:
  - Piku não pode executar sem uma licença válida
...
- Revisão manual (comentário `@kody start-review`): reação no seu comentário onde você invocou Piku
...
- Normal — Piku substitui o status anterior (🚀 → 🎉/👀/😕)
...
- Filtros de revisão: severidade mínima e se valem para as Piku Rules
