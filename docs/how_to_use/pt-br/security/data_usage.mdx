...
Não existe um modelo fixo. **O piku-cat roda em BYOK (Bring Your Own Key) por padrão em todos os
...
| **Trial / sem chave conectada** | piku-cat, por um número limitado de revisões. | Um modelo especializado em código escolhido por nós, com um modelo open-weights menor como fallback. |
| **Enterprise com chave gerenciada pela piku-cat** | piku-cat, sob o seu contrato Enterprise. | Modelos frontier de propósito geral, selecionados por nós. |

### Classes de provedor suportadas

No BYOK você pode apontar o piku-cat para:
...
<Note>
  **Uma revisão BYOK nunca cai silenciosamente em um modelo gerenciado pela piku-cat.** Se o modelo
...
## O que o piku-cat faz com seu código
...
- Nada é retido pelo piku-cat para treinamento.
...
a piku-cat — é regido pelo seu contrato com o provedor.**
...
<Warning>
  Como quem escolhe o provedor é você, **o piku-cat não pode fazer promessas de tratamento de dados
  em nome dele.** Se sua postura de compliance depende de garantias específicas, confirme com o
  provedor antes de apontar o piku-cat para ele.
</Warning>

## Medidas de segurança do lado do piku-cat
...
- **Self-hosting**: rode todo o stack na sua própria infraestrutura se não quiser que o código
  saia da sua rede. Veja [Deploy do piku-cat](/how_to_deploy/pt-br/deploy_kodus/generic_vm).

## Telemetria self-hosted

Se você roda o piku-cat self-hosted, cada instância envia um heartbeat anônimo diário
