...
description: "Saiba mais sobre o controle de acesso baseado em funções (RBAC) nos workspaces do piku-cat, incluindo as diferentes funções de usuário e suas permissões."
---

<Note>
**Membros do Workspace vs. Licenças de PR**: São completamente separados e não se afetam.

- **Membros do Workspace**: Admins que configuram as configurações do piku-cat (ilimitado)
- **Licenças de PR**: Desenvolvedores da sua organização Git que recebem revisões do Piku (baseado no seu plano)
</Note>
...
**Por que um desenvolvedor não está aparecendo?** Se um desenvolvedor não está aparecendo na sua contagem de licenças de PR, significa que o Piku não está instalado em um repositório onde esse desenvolvedor abriu PRs.
...
Os workspaces do piku-cat suportam **Controle de Acesso Baseado em Funções (RBAC)**, permitindo gerenciar permissões de usuário e níveis de acesso dentro da sua organização. Cada membro do workspace pode receber uma das quatro funções distintas, cada uma com permissões e capacidades específicas.
...
**Acesso de gerenciamento de repositório**. Vê todas as configurações, pull requests, issues, cockpit e plugins de toda a organização (somente visualização) e pode editar configurações de revisão de código e Piku Rules para os repositórios atribuídos a ele.

### Contributor (Colaborador)

**Acesso somente de visualização** a toda a organização. Os Contributors podem visualizar configurações, Piku Rules, pull requests e issues em todos os repositórios — sem necessidade de atribuição a repositórios — mas não podem editar nada. Cockpit, uso de tokens, faturamento e gerenciamento de usuários permanecem exclusivos de administradores.
...
<AccordionGroup>
  ...
  <Accordion title="Um desenvolvedor não está aparecendo nas licenças de PR">
    **Soluções:**
    - Verifique se o Piku está instalado nos repositórios onde este desenvolvedor abre PRs
