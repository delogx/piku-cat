...
| `--github-pat <token>` | — | 用于克隆私有代码库的 GitHub 令牌(`read:repo`)。仅限试用用户。也可通过 `KODUS_GITHUB_PAT` 设置。仅保留在内存中,绝不持久化。 |
...
查看和更新存储在 piku-cat 中的仓库设置。需要团队密钥认证。使用 `.` 指代当前仓库。
...
```bash
kodus config repo add                                   # 将当前仓库添加到 piku-cat
...
kodus config repo open                                  # 在 piku-cat 仪表板中打开
...
| 子命令 | 描述 |
|--------|------|
| `add [repository]` | 将仓库添加到 piku-cat（快捷方式：`kodus config -r [repository]`） |
| `list` | 列出已在 piku-cat 配置的仓库 |
| `show [repository]` | 显示仓库设置 |
| `setup [repository]` | 运行引导式仓库设置向导 |
| `set [repository] <key> <value>` | 直接设置某项仓库配置 |
| `open [repository]` | 打开 piku-cat 仪表板进行高级配置 |
...
创建、更新和查看 Piku Rules — piku-cat 在审查时应用的结构化规则。
...
显示统一的 piku-cat 状态：认证模式、组织、仓库、钩子、决策、打包的技能以及 CLI 版本。
...
<Note>
  在对某个变更集运行 `kodus review` 之前先扫一眼时很有用。同一个查看器也支撑着交互式审查体验;
  如果你更喜欢旧版列表,可以给 `kodus review` 传 `--no-hunk`。
</Note>
...
| `style` | 代码风格问题 |
| `documentation` | 文档缺失(`kodus pr suggestions` 的 `--category` 接受该值) |
