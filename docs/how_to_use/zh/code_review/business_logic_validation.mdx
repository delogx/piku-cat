...
业务逻辑验证让您确认拉取请求是否实现了规范、文档或工单中描述的预期行为。Piku 分析 PR 差异，引入引用的业务上下文，并在合并之前标记不匹配。

<img src="https://kodus.io/wp-content/uploads/2025/10/business-logic.gif" alt="使用业务逻辑验证的示例" />

## 前提条件
- 如果您希望 Piku **自动获取任务上下文**（来自 Jira、Linear、Notion、ClickUp 等工具），需要在工作区的[插件](/how_to_use/zh/code_review/plugins)页面中连接相应的插件。
...
启用后，Piku 在每次 PR 审查时自动运行业务逻辑验证，同时也支持按需命令。
...
2. 提及 Piku 并添加验证命令：`@kody -v business-logic ...`。
3. 内联提供规范内容或粘贴 Piku 可以通过可用插件获取的链接。
4. 提交评论并在同一线程中等待 Piku 的响应。

该命令仅从主对话框触发；内联建议内的回复将被忽略。

您可以随时重新运行验证——只需在新评论中再次发布命令即可。后续操作必须是新命令；不支持直接在 Git UI 中回复 Piku 的消息。
...
如果 Piku 无法访问资源或缺少权限，它会指出这一点，以便您可以调整访问权限或以其他方式提供信息。

## 任务上下文质量

Piku 在分析之前自动分类任务上下文的质量，这影响验证的深度：
...
Piku 不会发明需求。如果一个发现无法追溯到任务中的特定句子，则不会报告。

### 范围不匹配检测

如果 PR 差异似乎在与任务本身不同的领域工作，Piku 会检测到这是一个**范围不匹配**，并将其作为首要发现报告，而不是产生误导性的差距分析。
