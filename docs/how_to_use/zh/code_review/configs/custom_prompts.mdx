---
title: "自定义提示"
description: "为每个类别和建议的编写方式自定义 Piku 的审查提示以匹配团队的需求"
---

自定义提示让您完全控制 Piku 如何分析代码和沟通发现。定制 AI 在每个审查类别中查找的内容,以及如何呈现建议以匹配团队的标准和沟通风格。
...
控制 Piku 如何沟通每个单独的建议。这自定义了出现在 PR 评论中的消息格式和语气,例如:

```
新文件 'create-or-update-issues-parameter.dto.ts' 不遵循
公司的命名约定。根据 Piku 规则,所有新文件
必须使用驼峰命名法。请将文件重命名为 'createOrUpdateIssuesParameter.dto.ts'
以遵守标准。
```

您的自定义提示指示 Piku 如何编写建议消息。提示接收有关严重性级别、问题描述、建议代码以及是否为 Piku 规则违规的上下文。Piku 使用此上下文以及您的提示为每个建议生成适当的消息。

您可以根据团队的需求自定义语气、详细程度和沟通风格 - 从关键问题的详细解释到次要问题的简洁反馈。

## 在 `kodus-config.yml` 中设置
...
配置文件还接受一组界面上看不到的键 —— Piku 用来对发现进行分级的严重性评判标准:
...
<Warning>
  这会改变 Piku 对发现的*分级*,而不只是措辞 —— 一份糟糕的评判标准会在所有审查中悄悄压低或
  抬高严重性。请以随产品发布的默认值
...
<Note>
  随产品发布的 `default-kodus-config.yml` 中还包含一个 `v2PromptOverrides.level` 分组。出于向后
  兼容它仍会被接受,但**引擎已不再读取它** —— 设置它不会产生任何变化。只有
  `categories.descriptions`、`severity.flags` 和 `generation.main` 会进入审查。
</Note>

只有在启用 `kodusConfigFileOverridesWebPreferences` 之后该文件才会被读取(参见
[通用设置](/how_to_use/zh/code_review/configs/general))。

## 外部引用

所有自定义提示——类别、建议和 PR 摘要——都支持引用 Piku 可以访问的代码库中的真实文件。当您保存更新时,Piku 会自动检测外部引用并为将来的审查解析它们。

- 使用 `@file:path/to/file.ts` 指向您正在编辑的代码库中的文件;Piku 将首先尝试在本地找到路径。
- 在引用另一个代码库中的文件或在全局级别编辑提示时包含 `@repo:org/project`。
- 首选精确的 blob 样式路径而不是占位符,以便模型可以可靠地找到正确的文件。
- 保存后,Piku 在后台处理引用。检查提示编辑器旁边的状态指示器以确认解析何时完成。
...
<AccordionGroup>
  ...
  <Accordion title="类别提示和建议提示之间有什么区别?">
    **类别提示**控制 Piku 在每个审查类别中分析什么。**建议提示**控制 Piku 如何向团队呈现发现。您可以独立自定义两者。
  </Accordion>
  ...
  <Accordion title="如果我的建议模板有语法错误会怎样?">
    Piku 将回退到默认建议格式并在设置中通知您错误。
  </Accordion>

  <Accordion title="建议提示对 Piku 规则和标准建议都有效吗?">
