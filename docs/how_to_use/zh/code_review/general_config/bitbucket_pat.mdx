...
description: "生成 Piku 在 Bitbucket Cloud 上进行代码审查所需的 API Token,含需要授予的具体权限。"
---

## 简介

要使 Piku 能够对 Bitbucket Cloud 执行自动代码审查,您需要生成一个 **API 令牌**。此令牌允许 Piku 访问您的代码库、拉取请求、webhooks、工单、管道等,并具有下面列出的特定权限。
...
- 一旦使用所有必需的范围生成令牌,在启用 Bitbucket 集成时将其粘贴到 **Piku 集成/自动化设置屏幕**中。
- 如果 Piku 需要配置 webhooks 或监听拉取请求事件,请确保令牌具有 `admin:repository:bitbucket`(或等效权限)。
