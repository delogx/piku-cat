...
description: "生成 Piku 进行代码审查所需的 GitHub Fine-Grained PAT,包括如何解除对组织代码库的访问限制。"
---

## FGPAT 简介

要使 Piku 能够执行自动代码审查,您需要在 GitHub 中生成一个细粒度个人访问令牌(FGPAT)。此令牌允许 Piku 以受控方式安全地访问您的代码库并执行代码分析。
...
1. 在**细粒度个人访问令牌**下,选择**"允许通过细粒度个人访问令牌访问"**以启用 Piku 的安全访问。
...
6. **完成并保存令牌**:
   - 设置权限后,点击 `生成令牌`。
   - 确保复制生成的令牌并安全存储,因为您需要它来配置 Piku。

## 将令牌添加到 Piku

生成令牌后,将其粘贴到 Piku 配置屏幕中进行自动化设置。一旦您尝试启用自动化,模态框将自动打开。
