...
description: "Link sibling repositories so Piku can read contracts and APIs across services when a PR touches a boundary surface."
---

When your product is split across repositories — for example a frontend that calls a backend API, or a service that depends on a shared contracts package — a single-repo review can miss breakages at the boundary. **Linked repositories** give Piku optional, on-demand context from those sibling codebases during code review.
...
1. Piku first checks whether the PR diff touches anything boundary-shaped — a fast, fixed check that costs nothing.
...
3. Linked repos are **cloned lazily** only when Piku actually needs to read or search them.
...
5. Optionally set:
   - **Instructions** — free-text hint for Piku (e.g. “REST API this frontend consumes”).
...
- `ref` (optional): pin a branch, tag, or commit. When omitted, Piku uses the ref cascade.

Empty `linkedRepositories: []` means the feature is off.

## How the ref is chosen

For each linked repository, Piku tries refs in this order and uses the first one that clones successfully:
...
Even with links configured, Piku does **not** always open sibling repos. A cheap, deterministic gate scans **added lines** in the PR diff and activates only when it finds **boundary surface**, such as:
...
## What Piku can and cannot do with linked repos
...
- Link repositories outside the organization or that are not connected to piku-cat
