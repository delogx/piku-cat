...
description: "Learn how to connect Moonshot AI's Kimi K2.6 models to piku-cat — via the Moonshot Developer API or the Kimi Code Plan."
icon: "moon"
---

## How Moonshot works

Moonshot AI publishes the **Kimi** family of models (K2, K2.5, K2.6, K2.6 Coding). Kimi is particularly strong on long-context code understanding and agentic workflows, and the API is fully OpenAI-compatible — piku-cat talks to it via the `OpenAI Compatible` provider (or directly through the curated **Kimi K2.6 Coding** card in BYOK).
...
## Configure Moonshot in piku-cat

The primary flow is BYOK on piku-cat Cloud — the curated **Kimi K2.6 Coding** card handles the endpoint swap for you. Self-hosted users who prefer fixing the provider at the process level can use environment variables instead.
...
<Steps>
  ...
  <Step title="Paste your API key">
    Just the key. For Kimi Code Plan users, piku-cat pre-fills `maxConcurrentRequests=30` in Advanced settings (matches the documented cap).
  </Step>

  <Step title="Test & save">
    Click **Test & save**. piku-cat probes the endpoint with a cheap metadata call and persists the config on success. 401 means the key doesn't match the selected plan's endpoint.
  </Step>
</Steps>
...
<Note>
  Configure Kimi as **Main** and keep an OpenAI or Anthropic key as **Fallback** — if Moonshot returns 429 or 402, piku-cat fails over automatically.
</Note>
...
If you run piku-cat in Fixed Mode (single global provider, no per-org BYOK), configure Moonshot in the `.env` of your API + worker containers:
...
<AccordionGroup>
  ...
  <Accordion title="Region / connectivity">
    ...
    - Confirm outbound HTTPS to the chosen endpoint is allowed from your piku-cat deployment.
  </Accordion>
</AccordionGroup>
