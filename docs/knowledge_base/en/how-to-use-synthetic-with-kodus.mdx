...
description: "Learn how to connect Synthetic.new's subscription-based API — bundling Kimi, DeepSeek, Qwen Coder, GLM and more — to piku-cat."
icon: "infinity"
---
...
piku-cat reaches Synthetic through the OpenAI-compatible endpoint, so there are no adapter changes — just BYOK credentials.
...
## Configure Synthetic in piku-cat

The primary flow is BYOK on piku-cat Cloud. Self-hosted installs can use environment variables as a secondary option.

### Option 1 — BYOK on piku-cat Cloud (recommended)

1. In the piku-cat web UI, open **Settings → BYOK** ([app.kodus.io/organization/byok](https://app.kodus.io/organization/byok)).
...
4. Click **Test & save**. piku-cat probes the endpoint and persists the config on success. 401 means the key is wrong; 404 usually means the base URL or model path is wrong.
...
<Note>
  Configure Synthetic as **Main** and keep an OpenAI or Anthropic key as **Fallback** — when the 5-hour window fills up, piku-cat automatically fails over and reviews keep flowing.
</Note>

### Option 2 — Self-hosted (environment variables)

If you run piku-cat in Fixed Mode (single global provider, no per-org BYOK), configure Synthetic in the `.env` of your API + worker containers:
...
<Note>
  This path is only needed for self-hosted piku-cat installs that deliberately disable BYOK. If BYOK is enabled on your self-hosted instance, prefer **Option 1** — the UI-based flow is the same as on Cloud.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Long cold-start on first call">
    ...
    - If first-call latency is blocking piku-cat health checks, warm the model by issuing a tiny test request before opening PRs.
  </Accordion>
