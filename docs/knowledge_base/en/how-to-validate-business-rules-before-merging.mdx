...
piku-cat compares your PR diff against the task requirements and identifies:
...
With `business_logic: true` in your review options (enabled by default), piku-cat automatically validates business rules during every PR review when a task management plugin is connected.
...
- If the PR scope doesn't match the task, Piku flags a scope mismatch instead of speculating
