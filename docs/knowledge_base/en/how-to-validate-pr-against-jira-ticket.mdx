...
description: "Use piku-cat to compare your pull request against Jira acceptance criteria and catch missing requirements before merging."
---
...
3. Piku fetches the Jira ticket (title, description, acceptance criteria), reads the PR diff, and compares them

## What you get

Piku produces a structured report with:
...
With `business_logic: true` in your review options (enabled by default), Piku automatically validates against linked tasks during every PR review — no manual command needed.

## Tips for best results

- Add **acceptance criteria** to your Jira tickets — tasks with explicit criteria get criterion-by-criterion validation
- If the PR scope doesn't match the ticket, Piku detects the **scope mismatch** instead of producing misleading findings
