...
Linear issues often contain detailed requirements, acceptance criteria, and scope definitions. piku-cat can compare your PR against the Linear issue to catch gaps before merging.
...
3. Piku fetches the Linear issue context, reads the PR diff, and produces a gap analysis
...
When `business_logic: true` is enabled (default), Piku automatically retrieves task context from Linear during every PR review if the plugin is connected.
