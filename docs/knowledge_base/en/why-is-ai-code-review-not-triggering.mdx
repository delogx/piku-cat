...
description: "Common reasons and fixes when piku-cat doesn't automatically review your pull requests."
---

If piku-cat isn't reviewing your PRs, check these common causes:
...
piku-cat needs webhooks to know when a PR is opened or updated:
- Go to your Git provider's webhook settings
- Verify the piku-cat webhook URL is registered and active
- Check for failed delivery attempts

## 3. PR title matches an ignored keyword

If the PR title contains an ignored keyword, piku-cat skips the review:
...
By default, piku-cat reviews PRs targeting the default branch. If your PR targets a different branch, add it to `baseBranches`:
...
If review cadence is set to **Manual**, piku-cat only reviews when you comment `@kody start-review`.
