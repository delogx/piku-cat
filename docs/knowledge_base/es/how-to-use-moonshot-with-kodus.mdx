...
description: "Aprende a conectar los modelos Kimi K2.6 de Moonshot AI a piku-cat — a través del Developer API de Moonshot o el Kimi Code Plan."
icon: "moon"
---

## Cómo funciona Moonshot

Moonshot AI publica la familia de modelos **Kimi** (K2, K2.5, K2.6, K2.6 Coding). Kimi es particularmente robusto en comprensión de código de contexto largo y flujos de trabajo agénticos, y la API es totalmente compatible con OpenAI — piku-cat se comunica con ella a través del proveedor `OpenAI Compatible` (o directamente mediante la tarjeta curada **Kimi K2.6 Coding** en BYOK).
...
## Configurar Moonshot en piku-cat

El flujo principal es BYOK en piku-cat Cloud — la tarjeta curada **Kimi K2.6 Coding** maneja el cambio de endpoint por ti. Los usuarios con alojamiento autónomo que prefieren fijar el proveedor a nivel de proceso pueden usar variables de entorno en su lugar.
...
<Steps>
  ...
  <Step title="Pegar tu clave de API">
    Solo la clave. Para usuarios del Kimi Code Plan, piku-cat rellena previamente `maxConcurrentRequests=30` en la configuración avanzada (coincide con el tope documentado).
  </Step>

  <Step title="Probar y guardar">
    Haz clic en **Test & save**. piku-cat verifica el endpoint con una llamada de metadatos económica y persiste la configuración si tiene éxito. 401 significa que la clave no coincide con el endpoint del plan seleccionado.
  </Step>
</Steps>
...
<Note>
  Configura Kimi como **Principal** y mantén una clave de OpenAI o Anthropic como **Respaldo** — si Moonshot devuelve 429 o 402, piku-cat conmuta automáticamente.
</Note>
...
Si ejecutas piku-cat en Modo Fijo (proveedor global único, sin BYOK por organización), configura Moonshot en el `.env` de tus contenedores de API + worker:
...
<AccordionGroup>
  ...
  <Accordion title="Región / conectividad">
    ...
    - Confirma que el HTTPS saliente hacia el endpoint elegido esté permitido desde tu despliegue de piku-cat.
  </Accordion>
</AccordionGroup>
