...
description: "Aprende a conectar los modelos GLM de Z.AI — incluida la suscripción GLM Coding Plan — a piku-cat."
icon: "bolt"
---
...
Ambas rutas exponen un endpoint compatible con OpenAI, por lo que piku-cat se comunica con ellas a través del proveedor `OpenAI Compatible` (o directamente mediante la tarjeta curada **GLM 5.1** en BYOK).
...
## Configurar Z.AI en piku-cat

El flujo principal es BYOK en piku-cat Cloud — la tarjeta curada **GLM 5.1** maneja el cambio de endpoint por ti. Los usuarios con alojamiento autónomo que prefieren fijar el proveedor a nivel de proceso pueden usar variables de entorno en su lugar.

### Opción 1 — BYOK en piku-cat Cloud (recomendado)
...
<Steps>
  ...
  <Step title="Pegar tu clave de API">
    Solo la clave — nada más que configurar. Para usuarios del Coding Plan, piku-cat rellena previamente `maxConcurrentRequests=1` en la configuración avanzada, lo que coincide con los límites de los niveles Lite/Pro. Súbelo a 30 si estás en Max.
  </Step>

  <Step title="Probar y guardar">
    Haz clic en **Test & save**. piku-cat verifica el endpoint con una llamada de metadatos económica y persiste la configuración si tiene éxito. 401 significa que la clave no coincide con el endpoint del plan seleccionado; 404 significa que la URL base es incorrecta.
  </Step>
</Steps>
...
<Note>
  No necesitas envolver en namespace — piku-cat auto-envuelve bajo `openaiCompatible` (el provider activo) antes de enviar. Consulta [main BYOK doc → Custom JSON override](/how_to_use/es/byok#sobrescritura-con-json-personalizado) para más detalles.
</Note>
...
<Note>
  Configura GLM 5.1 como tu modelo **Principal** y mantén una clave de OpenAI o Anthropic como **Respaldo** para que las revisiones sigan funcionando cuando tu ventana de 5 horas del Coding Plan se agote. piku-cat conmuta automáticamente.
</Note>
...
Si ejecutas piku-cat en Modo Fijo (proveedor global único, sin BYOK por organización), configura Z.AI en el `.env` de tus contenedores de API + worker:
...
<Note>
  Esta opción solo es necesaria para instalaciones de piku-cat con alojamiento autónomo que deshabilitan deliberadamente el BYOK. Si el BYOK está habilitado en tu instancia con alojamiento autónomo, prefiere la **Opción 1** — la tarjeta curada maneja la lógica del endpoint por ti.
</Note>
...
Para la configuración completa con alojamiento autónomo (dominios, claves de seguridad, base de datos, webhooks, proxy inverso), sigue la [guía de despliegue en VM genérica](https://docs.kodus.io/how_to_deploy/es/deploy_kodus/generic_vm) y solo reemplaza el bloque LLM con el anterior.
...
<AccordionGroup>
  ...
  <Accordion title="'Too much concurrency' durante la revisión">
    - Los niveles Lite y Pro del Coding Plan normalmente permiten solo **1 solicitud concurrente**. piku-cat lo rellena por ti; súbelo solo en Max.
