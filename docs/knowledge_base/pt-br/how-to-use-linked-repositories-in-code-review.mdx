...
description: "Vincule repositórios irmãos para o Piku ler contratos e APIs entre serviços quando um PR toca uma superfície de fronteira."
---

Quando o produto está dividido em vários repositórios — por exemplo um frontend que chama uma API de backend, ou um serviço que depende de um pacote de contratos compartilhados — a revisão de um único repo pode deixar passar quebras na fronteira. **Repositórios vinculados** (*linked repositories*) dão ao Piku contexto opcional e sob demanda desses codebases irmãos durante a revisão de código.
...
1. O Piku analisa o diff do PR com um **gate determinístico de fronteira** (sem LLM).
...
3. Os repos vinculados são **clonados de forma lazy** só quando o Piku realmente precisa ler ou buscar neles.
...
5. Opcionalmente defina:
   - **Instructions** — dica em texto livre para o Piku (ex.: “API REST que este frontend consome”).
...
- `ref` (opcional): fixa branch, tag ou commit. Quando omitido, o Piku usa a cascata de refs.

`linkedRepositories: []` vazio significa que o recurso está desligado.

## Como a ref é escolhida

Para cada repositório vinculado, o Piku tenta refs nesta ordem e usa a primeira que clonar com sucesso:
...
Mesmo com vínculos configurados, o Piku **não** abre sempre os repos irmãos. Um gate barato e determinístico varre **linhas adicionadas** no diff do PR e só ativa quando encontra **superfície de fronteira**, como:
...
## O que o Piku pode e não pode fazer com repos vinculados
...
- Vincular repositórios fora da organização ou não conectados ao piku-cat
