...
description: "Aprenda como conectar os modelos GLM do Z.AI — incluindo a assinatura GLM Coding Plan — ao piku-cat."
icon: "bolt"
---
...
Ambos os caminhos expõem um endpoint compatível com OpenAI, então o piku-cat se comunica com eles via o provedor `OpenAI Compatible` (ou diretamente pelo card curado **GLM 5.1** em BYOK).
...
## Configurar o Z.AI no piku-cat

O fluxo principal é BYOK no piku-cat Cloud — o card curado **GLM 5.1** lida com a troca de endpoint por você. Usuários self-hosted que preferem fixar o provedor no nível do processo podem usar variáveis de ambiente.

### Opção 1 — BYOK no piku-cat Cloud (recomendado)
...
<Steps>
  ...
  <Step title="Colar sua chave de API">
    Apenas a chave — nada mais para configurar. Para usuários do Coding Plan, o piku-cat pré-preenche `maxConcurrentRequests=1` em Advanced settings, que corresponde aos limites dos tiers Lite/Pro. Aumente para 30 se você está no Max.
  </Step>

  <Step title="Test & save">
    Clique em **Test & save**. O piku-cat sonda o endpoint com uma chamada barata de metadados e persiste a configuração em caso de sucesso. 401 significa que a chave não corresponde ao endpoint do plano selecionado; 404 significa que a URL base está errada.
  </Step>
</Steps>
...
<Note>
  Não é necessário envolver em namespace — o piku-cat faz o auto-wrap sob `openaiCompatible` (o provider ativo) antes de enviar. Veja [main BYOK doc → Custom JSON override](/how_to_use/pt-br/byok#sobrescrita-via-json-customizado) para detalhes.
</Note>
...
<Note>
  Configure o GLM 5.1 como seu modelo **Main** e mantenha uma chave OpenAI ou Anthropic como **Fallback** para que as revisões continuem rodando quando sua janela de 5 horas do Coding Plan se esgotar. O piku-cat faz failover automaticamente.
</Note>
...
Se você executa o piku-cat em Fixed Mode (provedor global único, sem BYOK por organização), configure o Z.AI no `.env` dos seus containers de API e worker:
...
<Note>
  Este caminho só é necessário para instalações self-hosted do piku-cat que desabilitam deliberadamente o BYOK. Se o BYOK estiver habilitado na sua instância self-hosted, prefira a **Opção 1** — o card curado lida com a lógica do endpoint por você.
</Note>
...
Para a configuração completa self-hosted (domínios, chaves de segurança, banco de dados, webhooks, proxy reverso), siga o [guia de implantação em VM genérica](https://docs.kodus.io/how_to_deploy/pt-br/deploy_kodus/generic_vm) e substitua apenas o bloco de LLM pelo acima.
...
<AccordionGroup>
  ...
  <Accordion title="'Too much concurrency' no momento da revisão">
    - Os tiers Lite e Pro do Coding Plan geralmente permitem apenas **1 requisição simultânea**. O piku-cat pré-preenche isso para você; aumente apenas no Max.
