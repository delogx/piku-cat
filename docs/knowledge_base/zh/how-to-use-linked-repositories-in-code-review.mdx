...
description: "关联兄弟仓库，使 Piku 在 PR 触及边界表面时能读取服务间的契约与 API。"
---

当产品拆分在多个仓库中时——例如调用后端 API 的前端，或依赖共享契约包的服务——仅审查单个仓库可能会漏掉边界上的破坏。**关联仓库**（*linked repositories*）在代码审查期间为 Piku 提供这些兄弟代码库的可选、按需上下文。
...
1. Piku 用**确定性边界门控**（无 LLM）查看 PR diff。
2. 若 diff 看起来触及**契约表面**（DTO、export、字符串字面量、状态键、类 API 路径等），则启用跨仓库工具。
3. 仅当 Piku 真正需要读取或搜索时，才**懒克隆**关联仓库。
...
5. 可选设置：
   - **Instructions** — 给 Piku 的自由文本提示（例如：“此前端消费的 REST API”）。
...
- `ref`（可选）：固定 branch、tag 或 commit。省略时 Piku 使用 ref 级联。

空的 `linkedRepositories: []` 表示功能关闭。

## 如何选择 ref

对每个关联仓库，Piku 按以下顺序尝试 ref，并使用第一个成功克隆的：
...
即使已配置关联，Piku **也不会**总是打开兄弟仓库。廉价、确定性的门控扫描 PR diff 中的**新增行**，仅在发现**边界表面**时激活，例如：
