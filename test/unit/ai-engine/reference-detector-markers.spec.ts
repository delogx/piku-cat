...
describe('escapeRegExp', () => {
    ...
    it('leaves the current control markers intact (no behavior change)', () => {
        expect(escapeRegExp('@piku-sync')).toBe('@piku-sync');
        expect(escapeRegExp('@piku-ignore')).toBe('@piku-ignore');
    });
});

describe('stripControlMarkers', () => {
    it('removes @piku-sync / @piku-ignore case-insensitively, every occurrence', () => {
        expect(
            stripControlMarkers(
                'start @piku-sync middle @KODY-IGNORE end @piku-sync',
            ),
    ...
    it('does not mangle text that contains regex-special characters', () => {
        ...
        const text = 'version 1.2.3 (stable) and cost is $5 @piku-sync';
...
describe('ReferenceDetectorService.extractMarkers', () => {
    ...
    it('does NOT treat Kodus control markers as file references', () => {
        // Every rule file synced via the @piku-sync marker used to get a
        // spurious "file not found: @piku-sync" sync error on the rule.
        const markers = service.extractMarkers(
            'Rule body here.\n\n@piku-sync\n\nAlso @KODY-SYNC and @piku-ignore.',
            [],
        );
    ...
    it('still extracts real @file references', () => {
        const markers = service.extractMarkers(
            'See @AGENTS.md and @docs/standards.md for details. @piku-sync',
            [],
        );
        expect(markers).toContain('@AGENTS.md');
        expect(markers).toContain('@docs/standards.md');
        expect(markers).not.toContain('@piku-sync');
    });
});
...
describe('ReferenceDetectorService.detectReferences (LLM path)', () => {
    it('filters Kodus control markers from the model output', async () => {
        ...
        const refs = await service.detectReferences({
            requirementId: 'r1',
            promptText: 'rule body with @piku-sync and @docs/real-file.md',
